...
function is_test_group(msg)
  local var = false
  local tests = "tests"
  local chat = msg.to.id
   if data[tostring(tests)] then
      if data[tostring(tests)][tostring(msg.to.id)] then
         var = true
      end
         return var
   end
...
function ranks(user, group)
var = true
    if is_sudo2(user) then
       var = false
    elseif is_admin2(user) then
	   var = false
	 elseif is_owner2(user, group) then
	   var = false
	 elseif is_momod2(user, group) then
	   var = false 
	 end
	 return var
end

function ranks2(user, group)
var = ""
    if is_sudo2(user) then
       var = "سازنده ربات"
    elseif is_admin2(user) then
...
local cronned = load_from_file("data/expire.lua")
local function save_cron(msg, date, text)
   ...
   return save_file(cronned, "data/expire.lua")
 end
 ...
 local function delete_cron(date)
   ...
   save_file(cronned, "data/expire.lua")
 end
...
local function check_member_superrem2(cb_extra, success, result)
  ...
  groupss = string.gsub(receiver, "channel#id", "")
	  ...
	  for v,user in pairs(config.sudo_users) do
          sendmsg("user#id"..user, "شارژ این گروه تمام شد "..groupss.." "..redis:hget(receiver, "expire"))
      end
...
----
	...
	gkicklists = redis:scard("kicked_user")
        ...
        replymsg(extra.msg.id, text, ok_cb, false)
	end

function is_pattern(msg)
var = false
local chash = "cmuser2"..bot_divest..":"..msg.from.id
...
function enable_channel(receiver)
	if not config.disabled_channels then
		config.disabled_channels = {}
	end	
	if config.disabled_channels[receiver] == nil or config.disabled_channels[receiver] == false then
	   return sendmsg(receiver, "ربات در این گروه فعال است")
	end
	config.disabled_channels[receiver] = false
	save_config()
	return sendmsg(receiver, "ربات در این گروه فعال گردید" )
end

function disable_channel( receiver )
	if not config.disabled_channels then
		config.disabled_channels = {}
	end
	if config.disabled_channels[receiver] == true then
	   return sendmsg(receiver, "ربات در این گروه غیرفعال است")
	end
	config.disabled_channels[receiver] = true
	save_config()
	return sendmsg(receiver, "ربات در این گروه غیرفعال گردید" )
end
...
local function check_member_modadd(cb_extra, success, result)
  ...
  for k,v in pairs(result.members) do
    ...
    if member_id ~= our_id then
	  ...
	  for v,user in pairs(config.sudo_users) do
          sendmsg("user#id"..user, "گروهی به تازگی ادد شده است توسط "..usernamess)
      end
      local text = "گروه ذخیره شد!"
      return replymsg(msg.id, text, ok_cb, false)
    end
  end
end
...
local function check_member_super(extra, success, result)
	  ...
	  local text = "گروه ذخیره شد!"
      replymsg(msg.id, text, ok_cb, false)
	  for v,user in pairs(config.sudo_users) do
          return sendmsg("user#id"..user, "سوپر گروهی به تازگی ادد شده است توسط "..usernamess)
      end
end

local function add_grouptest(cb_extra, success, result)
  local limit = cb_extra.time
  local msg = cb_extra.msg
      -- Group configuration
      data[tostring(msg.to.id)] = {
        group_type = "گروه آزمایشی",
		long_id = msg.to.peer_id,
		moderators = {},
		owners = {},
        set_owner = nil,
	  ...
	  redis:set("Groupcm"..bot_divest..":"..msg.to.id, "off")
      local tests = "tests"
      if not data[tostring(tests)] then
        data[tostring(tests)] = {}
        save_data(data)
      end
      data[tostring(tests)][tostring(msg.to.id)] = msg.to.id
      save_data(data)
	  if not msg.from.username then
         usernamess = string.gsub(msg.from.print_name, "_", " ")..msg.from.username.." در "..msg.to.id
      else
	  ...
	  limit = limit * 3600
	  redis:setex("limit"..bot_divest..receiver, limit, true)
	  redis:sadd("testgroups"..bot_divest, receiver)
      sendmsg(receiver, "گروه آزمایشی برای "..cb_extra.time.." ساعت اضافه شد")
	  redis:hset("testgroup"..bot_divest..msg.to.id, "testgroup", msg.from.id)
	  for v,user in pairs(config.sudo_users) do
          return sendmsg("user#id"..user, "گروه آزمایشی جدید اضافه شد توسط "..usernamess)
      end
end
...
local function check_member_modrem(cb_extra, success, result)
  ...
  for k,v in pairs(result.members) do
    ...
    if member_id ~= our_id then
      ...
      for v,user in pairs(config.sudo_users) do
          sendmsg("user#id"..user, "گروهی به تازگی ریمو شده است توسط "..usernamess)
      end
...
local function check_member_superrem(cb_extra, success, result)
  local msg = cb_extra.msg
      ...
      replymsg(msg.id, text, ok_cb, false)
	  for v,user in pairs(config.sudo_users) do
          return sendmsg("user#id"..user, "سوپر گروهی به تازگی ریمو شده است توسط "..usernamess)
      end
end

local function rem_grouptest(cb_extra, success, result)
  local msg = cb_extra.msg
      data[tostring(msg.to.id)] = nil
      save_data(data)
      local tests = "tests"
      ...
      data[tostring(tests)][tostring(msg.to.id)] = nil
      save_data(data)
      return sendmsg(receiver, "گروه آزمایشی حذف شد")
end
...
function settings(msg, target)
	...
	if data[tostring(target)]["settings"] then
		...
		if not data[tostring(target)]["settings"]["lock_capmedia"] then
			data[tostring(target)]["settings"]["lock_capmedia"] = "no"
		end
  ...
  local text = "تنظیمات "..string.gsub(msg.to.title, "_", " ")
			 ...
			 .."\n پاک کننده چت : "..settings.lock_text
			 ...
			 .."\n پاک کننده مدیا(پیام های غیر متنی) : "..settings.lock_media
			 .."\n پاک کننده پیامهای کپشن دار : "..settings.lock_capmedia
...
if not v.first_name then
	...
	sendmsg(cb_extra.receiver, "در این گروه وجود ندارد")
	else
    sendmsg(cb_extra.receiver, text)
	end
end
...
local function gpinfoen(cb_extra, success, result)
if success == 0 then
return sendmsg(receiver, "I am not a admin") 
end
...
local lists = redis:scard(banlist)
 local mwarn = redis:get("mwarn"..bot_divest..":"..result.peer_id)
   ...
   lock_media = ""
		...
		if data[tostring(target)]["settings"]["lock_member"] then
		   if data[tostring(target)]["settings"]["lock_member"] == "yes" then
		      lock_member = "\nCleaner new member ✔️"
		   end
		end
		if data[tostring(target)]["settings"]["lock_join"] then
			if data[tostring(target)]["settings"]["lock_join"] == "yes" then
		      lock_join = "\nCleaner new member joined via link ✔️"
		   end
		end
		if data[tostring(target)]["settings"]["lock_link"] then
			if data[tostring(target)]["settings"]["lock_link"] == "yes" then
		      lock_link = "\nCleaner link messages ✔️"
		   end
		end
		if data[tostring(target)]["settings"]["lock_spam"] then
			if data[tostring(target)]["settings"]["lock_spam"] == "yes" then
		      lock_spam = "\nCleaner long messages ✔️"
		   end
		end
		if data[tostring(target)]["settings"]["lock_flood"] then
			if data[tostring(target)]["settings"]["lock_flood"] == "yes" then
		      lock_flood = "\nCleaner flood messages ✔️"
		   end
		end
		if data[tostring(target)]["settings"]["lock_arabic"] then
			if data[tostring(target)]["settings"]["lock_arabic"] == "yes" then
		      lock_arabic = "\nCleaner arabic messages ✔️"
		   end
		end
		if data[tostring(target)]["settings"]["lock_en"] then
			if data[tostring(target)]["settings"]["lock_en"] == "yes" then
		      lock_en = "\nCleaner english messages ✔️"
		   end
		end
		if data[tostring(target)]["settings"]["lock_sticker"] then
			if data[tostring(target)]["settings"]["lock_sticker"] == "yes" then
		      lock_sticker = "\nCleaner stikcers ✔️"
		   end
		end
		if data[tostring(target)]["settings"]["lock_contact"] then
			if data[tostring(target)]["settings"]["lock_contact"] == "yes" then
		      lock_contact = "\nCleaner share contacts ✔️"
		   end
		end
		...
		if data[tostring(target)]["settings"]["lock_text"] then
			if data[tostring(target)]["settings"]["lock_text"] == "yes" then
		      lock_text = "\nCleaner texts ✔️"
		   end
		end
		if data[tostring(target)]["settings"]["lock_audio"] then
			if data[tostring(target)]["settings"]["lock_audio"] == "yes" then
		      lock_audio = "\nCleaner voices and audio ✔️"
		   end
		end
		if data[tostring(target)]["settings"]["lock_video"] then
			if data[tostring(target)]["settings"]["lock_video"] == "yes" then
		      lock_video = "\nCleaner videos ✔️"
		   end
		end
		if data[tostring(target)]["settings"]["lock_document"] then
			if data[tostring(target)]["settings"]["lock_document"] == "yes" then
		      lock_document = "\nCleaner files ✔️"
		   end
		end
		if data[tostring(target)]["settings"]["lock_gif"] then
			if data[tostring(target)]["settings"]["lock_gif"] == "yes" then
		      lock_gif = "\nCleaner gifs ✔️"
		   end
		end
		if data[tostring(target)]["settings"]["lock_fwd"] then
			if data[tostring(target)]["settings"]["lock_fwd"] == "yes" then
		      lock_fwd = "\nCleaner forward message ✔️"
		   end
		end
		if data[tostring(target)]["settings"]["lock_bot"] then
			if data[tostring(target)]["settings"]["lock_bot"] == "yes" then
		      lock_bot = "\nCleaner API bots ✔️"
		   end
		end
		if data[tostring(target)]["settings"]["lock_tag"] then
			if data[tostring(target)]["settings"]["lock_tag"] == "yes" then
		      lock_tag = "\nCleaner hashtags and usernames ✔️"
		   end 
		end
		if data[tostring(target)]["settings"]["lock_reply"] then
			if data[tostring(target)]["settings"]["lock_reply"] == "yes" then
		      lock_reply = "\nCleaner replyes ✔️"
		   end
		end
		if data[tostring(target)]["settings"]["wlc"] then
			if data[tostring(target)]["settings"]["wlc"] == "yes" then
		      wlc = "\nCleaner welcome messages ✔️"
		   end
		end
		if data[tostring(target)]["settings"]["lock_settings"] then
			if data[tostring(target)]["settings"]["lock_settings"] == "yes" then
		      lock_settings = "\nStrict settings ✔️"
		   end
		end
		if data[tostring(target)]["settings"]["lock_unsup"] then
			if data[tostring(target)]["settings"]["lock_unsup"] == "yes" then
		      lock_unsup = "\nCleaner unsupport messages ✔️"
		   end
		end
		if data[tostring(target)]["settings"]["lock_media"] then
			if data[tostring(target)]["settings"]["lock_media"] == "yes" then
		      lock_media = "\nCleaner media messages ✔️"
		   end
		end
local title = "Info for <b> "..result.title.." </b>:\n\n"
local admin_num = "Total group admins :<i> "..result.admins_count.." </i>\n"
local user_num = "Total group users :<i> "..result.participants_count.." </i>\n"
local kicked_num = "Total users kicked :<i> "..result.kicked_count.." </i>\n"
local banlist = "Total users banned: <i> "..lists.." </i>\n"
local channel_id = "Id :<code> "..result.peer_id.." </code>\n"
local mwarn = "Warning limits :<i> "..mwarn.." </i>\n\nActive locks 🔒 : \n"
local locks = lock_tgservice..lock_member..lock_join..lock_link..
              lock_spam..lock_flood..lock_arabic..lock_en..lock_sticker..
		      lock_contact..lock_photo..lock_text..lock_audio..lock_video..
		      lock_document..lock_gif..lock_fwd..lock_bot..lock_tag..lock_unsup..
		      lock_reply..wlc..lock_settings..lock_media
local text = title..admin_num..user_num..kicked_num..banlist..channel_id..mwarn..locks
 return replymsg(cb_extra.msg.id, text, ok_cb, false)
end

local function gpinfofa(cb_extra, success, result)
if success == 0 then
return sendmsg(receiver, "ادمین نیستم") 
end
banlist = "banned:"..result.peer_id
local lists = redis:scard(banlist)
 local mwarn = redis:get("mwarn"..bot_divest..":"..result.peer_id)
   ...
   lock_porn = ""
		...
		if data[tostring(target)]["settings"]["lock_porn"] then
			if data[tostring(target)]["settings"]["lock_porn"] == "yes" then
		      lock_porn = "\nپاک کننده تصاویر +18 ✔️"
		   end
		end
...
local channel_id = "شناسه :<code> "..result.peer_id.." </code>\n"
local mwarn = "محدودیت اخطار:<i> "..mwarn.." </i>\n\nقفل های فعال 🔒 : \n"
local locks = lock_tgservice..lock_member..lock_join..lock_link..
              lock_spam..lock_flood..lock_arabic..lock_en..lock_sticker..
		      lock_contact..lock_photo..lock_text..lock_audio..lock_video..
		      lock_document..lock_gif..lock_fwd..lock_bot..lock_tag..lock_unsup..
		      lock_reply..wlc..lock_settings..lock_media..lock_porn
local text = title..admin_num..user_num..kicked_num..banlist..channel_id..mwarn..locks
 return replymsg(cb_extra.msg.id, text, ok_cb, false)
end

function callback_about(cb_extra, success, result)
if success == 0 then
return sendmsg(receiver, "ادمین نیستم") 
end
local about = "توضیحات گروه :\n<code> "..result.about.." </code>"
 return sendmsg(receiver, about) 
end
...
deleted = deleted + 1
 ...
 return sendmsg(receiver, "در این گروه هیچ فرد دیلیت اکانتی وجود ندارد.") 
 elseif tonumber(deleted) == 1 then
 return sendmsg(receiver, "1 نفر چون دیلیت اکانت کرده بود ریمو شد") 
 else
 return sendmsg(receiver, deleted.." نفر چون دیلیت اکانت بودند ریمو شدند.") 
 end
 end

function callback_who(cb_extra, success, result)
local text = "کاربران "..cb_extra.title
local i = 1
for k,v in pairsByKeys(result) do
...
if success == 0 then
return sendmsg(receiver, "من ادمین نشدم ")
end
--vardump(result)
local text = "لیست افراد ریمو شده "..cb_extra.title.."\n\n"
local i = 1
for k,v in pairsByKeys(result) do
if not v.print_name then
	name = " "
...
if tonumber(i) == 0 then
 return sendmsg(receiver, "در این گروه هیچ ربات Api وجود ندارد") 
 else
 return sendmsg(receiver, i.." ربات Api ریمو شدند") 
 end
end
...
function clean_bots3(extra, success, result)
	...
	for k,v in pairsByKeys(result) do
		local bot_id = v.peer_id
		if not is_momod2(bot_id, channel_id) then
		   kick_user(bot_id, channel_id)
		end
	end
end
...
for k,v in pairsByKeys(result.members) do
 ...
 return sendmsg(receiver, i.." نفر چون دیلیت اکانت بودند ریمو شدند") 
 end
 end
...
local function set_bot_photo(msg, success, result)
  if success then
    ...
    sendmsg(receiver, "Photo changed!", ok_cb, false)
    redis:del("bot:photo")
  else
    print("Error downloading: "..msg.id)
    sendmsg(receiver, "Failed, please try again!", ok_cb, false)
  end
end
...
local function set_supergroup_photo(msg, success, result)
  ...
  if success then
	...
	if msg.to.type == "channel" then
       channel_set_photo("channel#id"..msg.to.id, file, ok_cb, false)
	elseif msg.to.type == "chat" then
	   chat_set_photo("chat#id"..msg.to.id, file, ok_cb, false)
	end
    data[tostring(msg.to.id)]["settings"]["set_photo"] = file
    save_data(data)
  else
    sendmsg(receiver, "خطا!", ok_cb, false)
  end
end
...
local function unwarn_user(msg, names, user_id, chat_id)
 local mwarn = redis:get("mwarn"..bot_divest..":"..chat_id)
        ...
        if not warns2 then
		   return replymsg(msg, names.." تاکنون اخطاری دریافت نکرده است", ok_cb, false)
		else
		   ...
		   return replymsg(msg, text, ok_cb, false)
           end
...
local mwarn = redis:get("mwarn"..bot_divest..":"..chat_id)
 ...
 function welcome_group(msg)
action = msg.action.type
if msg.action.type == "chat_rename" then
data[tostring(msg.to.id)]["settings"]["set_name"] = msg.to.title
save_data(data)
return sendmsg(receiver, "اسم جدید با موفقیت ثبت شد", ok_cb, false)
end
...
if (action == "chat_add_user" or action == "chat_del_user" or action == "chat_add_user_link" ) then
  if msg.action.type == "chat_add_user" then
	  ...
	  if is_momod2(msg.from.id, msg.to.id) then
	     sendmsg(receiver, string.gsub(msg.action.user.print_name, "_", " ").." بن یا بن ال است", ok_cb, false)
	  end
	     ...
	     if redis:get(banadder) then
		  ...
		  if tonumber(banadder) >= 4 and not is_momod(msg) then
            ...
            return replymsg(msg.id, msg.from.print_name:gsub("_"," ").."به دلیل ادد کردن فرد بن شده بیش از 3 بار بن شد ", ok_cb, false)
		  end
	    end
...
if data[tostring(msg.to.id)] then
	    ...
	    if string.sub(msg.action.user.username:lower(), -3) == "bot" then
	     ...
	     if redis:get(botadder) then
		  ...
		  if tonumber(botadder) >= 4 then
		    ...
		    --sendmsg(receiver, "ربات @"..msg.action.user.username.." توسط "..names.." اضافه شد!!!!!!")
   ...
   local value = redis:hget(hash, msg.action.user.id)
	  ...
	  if data[tostring(msg.to.id)]["group_wlc"] == nil then
		 ...
		 return replymsg(msg.id, group_wlc, ok_cb, false)
		 end
	end
	 if msg.action.type == "chat_add_user_link" then
	  if data[tostring(msg.to.id)]["settings"]["lock_join"] == "yes" and not is_momod2(msg.from.id, msg.to.id) then
	  return kick_user(msg.from.id, msg.to.id)
	  end
	...
	if msg.action.type == "chat_del_user" then
	  ...
	  local value = redis:hget(hash, msg.action.user.id)
		...
		if tonumber(msg.action.user.id) == tonumber(msg.from.id) then
           ...
           return replymsg(msg.id, "خدانگهدار "..name, ok_cb, false)
...
function nuditycheck(msg, success, result)
local gwarns = "warnall"..bot_divest..":"..msg.to.id..":"..msg.from.id

  if success then
  local file = 'data/tmp/'..string.sub(result, 38)
  os.rename(result, file)
  ...
  local jdat = json:decode(res)
    ...
    if jdat.nudity.result then
	   if data[tostring(msg.to.id)]["settings"].lock_porn == "yes" then
	          if data[tostring(msg.to.id)]["settings"].lock_settings ==  "yes" then
	             redis:incr(gwarns)
              end
	          delmsg(msg.id)
			  if msg.from.username ~= nil then username = "@"..msg.from.username else username = string.sub(msg.from.print_name:gsub("_"," "), 1, 100) end
	          sendmsg(receiver, "کابر "..username.." از ارسال تصاویر +18 به گروه خودداری کنید")
	   elseif data[tostring(msg.to.id)]["settings"].lock_porn == "kick" then
	          kick_user(msg.from.id, msg.to.id)
		      if msg.from.username ~= nil then username = "@"..msg.from.username else username = string.sub(msg.from.print_name:gsub("_"," "), 1, 100) end
	          sendmsg(receiver, "کابر "..username.." به دلیل ارسال تصاویر مستحجن اخراج گردید")
		  end
       end
    end
end
...
--------------------------------------------------#process -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
local pmtext = [[احتراما به ربات ضد اسپم و ضد لینک یوبی
خیر مقدم میگوییم

این ربات توسط @VALTMAN ساخته شده است

<b>با تضمین 24 ساعت انلاینی ربات </b>😳😳

<code>دارای بهترین امکانات و بهترین سورس </code>

رباتهای ویژه 15000 تومان 
رباتهای معمولی 12000 تومان

جهت خرید به👈 @ValtMan مراجعه کنید یا اگر ریپورت هستید به گروه ساپورت یا @UBsupportbot مراجعه کنید
...
درگاه پرداخت امن و بدون دردسر 👇
https://www.payping.ir/teleub

لینک ساپورت : ]]..redis:hget("support", "supportlink")..[[


ادرس کانال ربات : https://telegram.me/joinchat/CNu6qD7nHPmNvSUznLWALw ==> @UB_CH

تمام به روزرسانی ها در این کانال قرار خواهد گرفت



در پایان برای شماارزوی موفقیت داریم
]]

local function private(msg)
redis:incr("msgs:"..msg.from.id..":"..msg.to.id)
  local users2 = "pvlimite"..bot_divest..":"..msg.from.id
  local hash = "user"..bot_divest..":"..msg.from.id..":msgs"
  local msgs = tonumber(redis:get(hash) or 0)
     mark_read(receiver, ok_cb, false)
     redis:sadd("PM:"..msg.from.id, msg.from.id)
    if not redis:get(users2) then
	    if not is_sudo(msg) and is_admin1(msg) then
	       return msg
	    end
	 	 local users = "pvusers"
	 	 redis:sadd(users, msg.from.id)
	 	 redis:setex(users2, 3600, true)
	 	 sendmsg(receiver, msg.from.print_name:gsub("_"," ").." "..pmtext, ok_cb, false)
       for k,v in pairs(redis:smembers("adds"..bot_divest)) do
           fwd_msg(receiver, v, ok_cb, false)
       end
	 end
 	 if msgs >= 5 and not is_admin1(msg)  then
	    sendmsg("user#id"..msg.from.id, "به دلیل ارسال پیام مکرر بیش از 4 بار بلاک میشوید")
        block_user("user#id"..msg.from.id, ok_cb, false)
     end
end
----------------------------#run---------------------------------------------------------------------------------------------------------------------------------------------
local function matches(msg, matches, ln)
--------------------------------------------------#chack time msg
local chash = "cmuser"..bot_divest..":"..msg.from.id
...
if matches[1] == "expire" and is_momod(msg) then
  ...
  for date, values in pairs(cronned) do
 if not date then
    return "جناب برای این گروه شارژ نشده است"
 end
 end
end
...
if matches[1] == "support" then
      ...
      local text = "<b>لینک گروه پشتیانی TeleUB </b>:\n"..support
      return replymsg(msg.id, text, ok_cb, false)
end

if matches[1] == "setsupport" and msg.text:match("(https://telegram.me/joinchat/%S+)") and is_sudo(msg) then
TEXT = msg.text:gsub("[Ss][Ee][Tt][Ss][Uu][Pp][Pp][Oo][Rr][Tt]", "")
	  redis:hset("support", "supportlink", string.match(TEXT, "https://telegram.me/joinchat/%S+") )
...
if matches[1] == "tosuper" and is_admin1(msg) then
   if is_super_group(msg) then
	  return replymsg(msg.id, "سوپر گروه است", ok_cb, false)
   end 
...
if type(msg.mention) ~= "nil" then
   return replymsg(msg.id, "برای ربات مجاز نیست")
end

if matches[1] == "add" and is_admin1(msg) then
 if to_chat then
   if is_group(msg) then
	return replymsg(msg.id, "گروه معمولی ذخیره شده است", ok_cb, false)
   end
...
elseif to_super then
    if is_super_group(msg) then
	   return replymsg(msg.id, "سوپر گروه ذخیره شده است")
	end
	if is_test_group(msg) then
	   return replymsg(msg.id, "گروه تست قبلا اضافه شده است", ok_cb, false)
	end
...
if matches[1] == "rem" and is_admin1(msg) then
 if to_chat then
   if not is_group(msg) then
	return replymsg(msg.id, "گروه معمولی ذخیره نشده است", ok_cb, false)
   end
   return chat_info(receiver, check_member_modrem, {receiver = receiver, msg = msg})
elseif to_super then
    if not is_super_group(msg) then
	 return replymsg(msg.id, "سوپر گروه ذخیره نشده است", ok_cb, false)
	end
  return channel_get_users(receiver, check_member_superrem, {receiver = receiver, msg = msg})
 end
end

if matches[1] == "addtest" and string.match(matches[2], "^%d+$") and is_admin1(msg) then
if to_super then
    if is_super_group(msg) then
	   return replymsg(msg.id, "سوپر گروه ذخیره شده است", ok_cb, false)
	end
	if is_test_group(msg) then
	   return replymsg(msg.id, "گروه تست قبلا اضافه شده است", ok_cb, false)
	end
	if tonumber(matches[2]) > 5 then
	   return replymsg(msg.id, "حداکثر مقدار 5 ساعت میباشد", ok_cb, false)
	end
	if 0 >= tonumber(matches[2]) then
	   return false
	end
	time = matches[2]
	     --savelog(msg.to.id, "testgroup added by "..msg.from.print_name:gsub("_",""))
  return channel_get_users(receiver, add_grouptest, {receiver = receiver, time = time, msg = msg})
 end
end
if matches[1] == "remtest"  and is_admin1(msg) then
if to_super then
    if not is_test_group(msg) then
	   return replymsg(msg.id, "گروه تست نیست", ok_cb, false)
	end
  return channel_get_users(receiver, rem_grouptest, {receiver = receiver, msg = msg})
 end
end
--------------------------------------------------#remove groups by id
if matches[1] == "rem" and matches[2] and is_admin1(msg) then
			...
			local hashaa = "chat:"..matches[2]..":badword"
            redis:del(hashaa, "1")
	        redis:del("Groupcm"..bot_divest..":"..matches[2], true)
	        redis:del("mwarn"..bot_divest..":"..matches[2], true)
			...
			save_data(data)
			return sendmsg(receiver, "گروه "..matches[2].." حذف شد")
end
--------------------------------------------------#leave
 if matches[1] == "leave" and not matches[2] and is_admin1(msg) then
	if is_super_group(msg) then
	return replymsg(msg.id, "با عرض پوزش جناب این گروه ذخیره شده است\nنمیتونم لیو بدم :()", ok_cb, false)
	end
	local bot_id = our_id
	if msg.to.type == "chat" then
    ...
    return reload_plugins(true)
        ...
        if matches[1] == "gpinfo" and is_momod(msg) then
		  if to_chat then
			...
			channel_info(receiver, gpinfo, {receiver = receiver, msg = msg})
		end
		...
		--[[if matches[1] == "owner" then
	        ...
	        for v,user in pairs(config.sudo_users) do
                sendmsg("user#id"..user, "سوپر گروه اونر ندارد |"..msg.to.id)
            end
		...
		if success == 0 then
		return replymsg(extra.msg.id, "test", ok_cb, false)
		end
		 ...
		 local text = "صاحب گروه "..names.." میباشد"
		 return replymsg(extra.msg.id, text, ok_cb, false)
          end
		...
		if matches[1] == "owners" or matches[1] == "ownerlist" then
		     local group_owner = data[tostring(msg.to.id)]["set_owner"]
                   if next(data[tostring(msg.to.id)]["owners"]) == nil then
                      return "لیست مدیران تشکلیل نشده است"
                   end
                   if not group_owner then
                      return "صاحب اصلی برای این گروه انتخاب نشده است با پشتیبانی ربات تماس بگیرید"
                   end
		...
		if matches[1] == "who" and not matches[2] and is_momod(msg) then
		   if to_chat then
			...
			title = msg.to.title
			return channel_get_users(receiver, callback_who, {receiver = receiver, msg = msg, title = title})
		end
		...
		if matches[1] == "kicked" and is_momod(msg) then
		  ...
		  title = msg.to.title
			return channel_get_kicked(receiver, callback_kicked, {receiver = receiver, msg = msg, title = title})
		end
     ...
     if matches[1] == "newlink" and is_momod(msg)then
			...
			if to_super then
			   ...
			   return export_chat_link(receiver, callback_link, {msg = msg})
			end
		...
		if matches[1] == "slink" and is_momod(msg) then
			...
			--local url , res = http.request("http://api.yon.ir/?url="..URL.escape(group_link))
			local llink = http.request("http://llink.ir/yourls-api.php?signature=a13360d6d8&action=shorturl&url="..group_link.."&format=simple")
            --[[if res ~= 200 then 
               return "خطا\nاز link استفاده کنید" 
            end]]
			--local jdat = json:decode(url)
			--savelog(msg.to.id, "لینک گروه درخواست شد توسط "..msg.from.print_name:gsub("_",""))
			--local text = "لینک گروه "..msg.to.title.." بصورت کوتاه :\n yon.ir/"..jdat.output
			local text = "لینک گروه "..msg.to.title.." بصورت کوتاه :\n "..llink:gsub("http://", "")
			return replymsg(msg.id, text, ok_cb, false)
		end
--------------------------------------------------#setadmin
if matches[1] == "setadmin" and is_owner1(msg) then
    ...
    if type(msg.reply_id)~="nil" then
	   local function setadmin_reply(extra , success, result)
	      if success == 0 then
			return replymsg(extra.msg.reply_id, "من سازنده گروه نیستم", ok_cb, false)
		  end
          ...
          return replymsg(extra.msg.reply_id, text, ok_cb, false)
        end
...
if matches[1] == "demoteadmin" and is_owner1(msg) then
    ...
    elseif matches[1] == "demoteadmin" and matches[2] and string.match(matches[2], "^%d+$") then
         channel_set_admin("channel#id"..chat_id, "user#id"..matches[2], ok_cb, false)
		 sendmsg(receiver, "[ "..matches[2].." ] از ادمینی گروه خارج شد")
...
if matches[1] == "setowner" and is_owner1(msg) then
   if not is_admin1(msg) then
      return "شما اجازه چنین کاری را ندارید !!\nدرصورت تمایل با پشتیبانی هماهنگ کنید"
   end
        if type(msg.reply_id) ~= "nil" then
		 local function setowner_reply(extra , success, result)
		  ...
		  if data[tostring(extra.chat_id)]["set_owner"] == tostring(result.from.peer_id) then
             return replymsg(extra.msg.reply_id, names.." ایشان صاحب گروه است", ok_cb, false)
          end
		  if ranks(result.from.peer_id, result.to.peer_id) then
		     return replymsg(extra.msg.reply_id, "ایشان دارای مقام بالاتری است", ok_cb, false)
		  end
		  ...
		  return get_message(msg.reply_id, setowner_reply, {msg = msg, lang = lang})
	elseif matches[1] == "setowner" and matches[2] and string.match(matches[2], "^%d+$") then
	      if ranks(msg.from.id, msg.to.id) then
		     return replymsg(msg.id, "ایشان دارای مقام بالاتری است", ok_cb, false)
		  end
   ...
   elseif matches[1] == "setowner" and matches[2] and not string.match(matches[2], "^%d+$") then
		...
		return false
	      ...
	      if data[tostring(extra.chat_id)]["set_owner"] == tostring(result.peer_id) then
             return replymsg(extra.msg.id, "@"..result.username.." ایشان صاحب گروه است", ok_cb, false)
          end
	      if not ranks(result.peer_id, extra.chat_id) then
		     return replymsg(extra.msg.id, "ایشان دارای مقام بالاتری است", ok_cb, false)
		  end
          data[tostring(extra.chat_id)]["set_owner"] = tostring(result.peer_id)
          save_data(data)
          local text = " [ ".."@"..result.username.." ] از این به بعد صاحب اصلی گروه هستند\nلطفا help را وارد کنید"
          return replymsg(extra.msg.id, text, ok_cb, false)
        end
				local username = string.gsub(matches[2], "@", "")
...
if matches[1] == "setowners" and is_owner1(msg) then 
		local group_owner = data[tostring(msg.to.id)]["set_owner"]
		  ...
		  if data[tostring(extra.chat_id)]["owners"][tostring(user_id)] then
		     return sendmsg(receiver, names.." جزء صاحبان گروه است")
		  end
		  if not ranks(result.from.peer_id, extra.to.peer_id) then
		     return replymsg(extra.msg.reply_id, "ایشان دارای مقام بالاتری است", ok_cb, false)
		  end
   ...
   elseif matches[1] == "setowners" and matches[2] and string.match(matches[2], "^%d+$") then
		...
		if not ranks(msg.from.id, msg.to.id) then
		     return replymsg(extra.msg.id, "ایشان دارای مقام بالاتری است", ok_cb, false)
		  end
		 data[tostring(extra.chat_id)]["owners"][tostring(user_id)] = names
		 save_data(data)
	  ...
	  if success == 0 then
		return false
		  ...
		  local user_id = result.peer_id
		  if data[tostring(extra.chat_id)]["owners"][tostring(user_id)] then
          return replymsg(extra.msg.id, "@"..result.username.." جزء صاحبان گروه است", ok_cb, false)
          end
		  if not ranks(result.peer_id, extra.chat_id) then
		     return replymsg(extra.msg.id, "ایشان دارای مقام بالاتری است", ok_cb, false)
		  end
...
if matches[1] == "remowners" and is_owner1(msg) then 
        if type(msg.reply_id) ~= "nil" then
		 local function remowners_reply(extra , success, result)
          ...
          return replymsg(extra.msg.id, names.." در لیست صاحبان گروه نیست", ok_cb, false)
          end
		...
		if not data[tostring(extra.chat_id)]["owners"][tostring(user_id)] then
        return sendmsg(receiver, names.." در لیست صاحبان گروه نیست")
        end
		 data[tostring(extra.chat_id)]["owners"][tostring(user_id)] = nil
		 save_data(data)
		...
		return false
		  ...
		  local user_id = result.peer_id
		  if not data[tostring(extra.chat_id)]["owners"][tostring(user_id)] then
          return replymsg(extra.msg.id, "@"..result.username.." در لیست صاحبان گروه نیست", ok_cb, false)
          end
		  data[tostring(extra.chat_id)]["owners"][tostring(user_id)] = nil
		  save_data(data)
...
if matches[1] == "promote" and is_owner(msg) then 
        if type(msg.reply_id) ~= "nil" then
		 local function promote_reply(extra , success, result)
		  ...
		  return sendmsg(receiver, names.." مدیر است")
		  end
		  if not ranks(result.from.peer_id, extra.chat_id) then
		     return replymsg(extra.msg.reply_id, "ایشان دارای مقام بالاتری است", ok_cb, false)
		  end
		...
		return false
         ...
         local group = tostring(extra.chat_id)
		  ...
		  local user_id = result.peer_id
		  if data[group]["moderators"][tostring(user_id)] then
          return replymsg(extra.msg.id, "@"..result.username.." مدیر است", ok_cb, false)
          end
		  if not ranks(result.peer_id, extra.chat_id) then
		     return replymsg(extra.msg.id, "ایشان دارای مقام بالاتری است", ok_cb, false)
		  end
...
if matches[1] == "demote" and is_owner(msg) then 
        if type(msg.reply_id) ~= "nil" then
		 local function demote_reply(extra , success, result)
          ...
          return replymsg(extra.msg.id, names.." مدیر نیست", ok_cb, false)
          end
		 ...
		 function demote_id(extra, success, result)
        ...
        local group = tostring(extra.chat_id)
		if not data[group]["moderators"][tostring(user_id)] then
        return sendmsg(receiver, names.." مدیر نیست ")
        end
		 data[group]["moderators"][tostring(user_id)] = nil
		 save_data(data)
         ...
         local group = tostring(extra.chat_id)
		  local user_id = result.peer_id
		  if not data[group]["moderators"][tostring(user_id)] then
          return sendmsg(receiver, "@"..result.username.." مدیر نیست")
          end
		  data[group]["moderators"][tostring(user_id)] = nil
		  save_data(data)
          local text = "@"..result.username.." [ "..result.peer_id.." ] از مدیریت بر کنار شد"
          return sendmsg(receiver, text)
        end
		  local username = string.gsub(matches[2], "@", "")
...
if matches[1] == "muteall" and is_admin1(msg)  then
    if type(msg.reply_id)~="nil" then
      local function muteall_reply(extra , success, result)
		  ...
		  return replymsg(extra.msg.id, "ایشان "..ranks2(user_id, group_id).." است", ok_cb, false)
		  end
		  if is_muteall_user(user_id) then
		  return replymsg(extra.msg.reply_id, names.." از چت در گروه های یوبی محروم است", ok_cb, false)
		  end
    ...
    elseif matches[1] == "muteall" and matches[2] and string.match(matches[2], "^%d+$") then
		...
		if is_muteall_user(matches[2]) then
		  return replymsg(msg.id, matches[2].." از چت در گروه های یوبی محروم است", ok_cb, false)
		  end
      ...
      elseif matches[1] == "muteall" and matches[2] and not string.match(matches[2], "^%d+$") then
		...
		return false
		 ...
		 local names = "@"..result.username
		 local user_id = result.peer_id
		 local group_id = extra.chat_id
		  if is_momod2(user_id, extra.chat_id) then
		  return replymsg(extra.msg.id, "ایشان "..ranks2(user_id, group_id).." است", ok_cb, false)
		  end
		  if is_muteall_user(user_id) then
		  return replymsg(extra.msg.id, names.." از چت در گروه های یوبی محروم است", ok_cb, false)
		  end
		 muteall_user(user_id)
 ...
 if matches[1] == "unmuteall" and is_admin1(msg)  then
    if type(msg.reply_id)~="nil" then
      local function unmuteall_reply(extra , success, result)
		  ...
		  return replymsg(extra.msg.reply_id, names.." از چت در گروه های یوبی محروم نیست", ok_cb, false)
		  end
    ...
    elseif matches[1] == "unmuteall" and matches[2] and string.match(matches[2], "^%d+$") then
      ...
      elseif matches[1] == "unmuteall" and matches[2] and not string.match(matches[2], "^%d+$") then
		...
		return false
		 ...
		 local user_id = result.peer_id
		  if is_momod2(user_id, extra.chat_id) then
		  return replymsg(extra.msg.id, "ایشان "..ranks2(user_id, extra.chat_id).." است", ok_cb, false)
		  end
		  if not is_muteall_user(user_id) then
		  return replymsg(extra.msg.id, names.." از چت در گروه های یوبی محروم نیست", ok_cb, false)
		  end
		 unmuteall_user(user_id)
...
if matches[1] == "clean" and is_momod(msg) then
		...
		--------------------------------------------------#clean lists
            if matches[2] == "banlist" and is_owner(msg)  then
			   ...
			   return sendmsg(receiver, "لیست افراد بن شده خالی است")
			   end
               redis:del(hash)
			   --savelog(msg.to.id, "لیست افراد محروم از گروه درخواست شد توسط "..msg.from.print_name:gsub("_",""))
               sendmsg(receiver, "لیست افراد بن شده خالی شد")
			end
            if matches[2] == "gbanlist" and is_sudo(msg) then
               local hash = "gbanned"
			   if redis:scard(hash) == 0 then
			   return sendmsg(receiver, "لیست افراد بن ال شده خالی است")
			   end
               redis:del(hash)
               sendmsg(receiver, "لیست افراد بن ال شده در یوبی خالی شد")
			end
            if matches[2] == "gmutelist" and is_sudo(msg) then
               local hash = "muteall_user"
			   if redis:scard(hash) == 0 then
			   return sendmsg(receiver, "لیست افراد محروم از چت در گروه های یوبی خالی است")
			   end
               redis:del(hash)
               sendmsg(receiver, "لیست افراد محروم از چت در گروه های یوبی خالی شد")
            end
	end
--------------------------------------------------#flood set settings
if matches[1] == "setflood" and is_momod(msg) then
			...
			return replymsg(msg.id, "تعداد پیام های مکرر به "..matches[2].." محدود شد", ok_cb, false)
		end
		if matches[1] == "settimeflood" and is_momod(msg) then
			...
			return replymsg(msg.id, "زمان تعداد پیام های مکرر "..matches[2].." محدود شد", ok_cb, false)
end
--------------------------------------------------#public settings
...
if matches[1] == "mute" and matches[2] == "all" and is_momod(msg) and matches[3] and not matches[4] then
 if  tonumber(matches[3]) < 1 or tonumber(matches[3]) > 120 then
		return "فقط بین 1 دقیقه تا 120 دقیقه مجاز به میوت همه پیام ها میباشید."
    		...
    		redis:setex("muteall:"..receiver, time, true)
			redis:sadd("groupmuteall"..bot_divest, receiver)
...
if matches[1] == "mute" and matches[2] == "all" and is_momod(msg) then
			...
			--savelog(msg.to.id, "پاک کننده همه پیام های گروه برای 7 ساعت فعال شد توسط "..msg.from.print_name:gsub("_",""))
...
--------------------------------------------------#muteallgroup
if matches[1] == "lock" and matches[2] == "all" and is_momod(msg) and matches[3] and not matches[4] then
 if  tonumber(matches[3]) < 1 or tonumber(matches[3]) > 120 then
		return "فقط بین 1 دقیقه تا 120 دقیقه مجاز به میوت همه پیام ها میباشید."
end
			local time = matches[3] * 60
			...
			--savelog(msg.to.id, "پاک کننده همه پیام های گروه برای "..matches[3].." دقیقه فعال شد توسط "..msg.from.print_name:gsub("_"," "))
            return "پاک کننده همه پیام ها برای <b>"..matches[3].."</b> دقیقه فعال گردید\nپایان زمان میوت ال بشما اطلاع داده میشود"
elseif matches[1] == "lock" and matches[2] == "all" and is_momod(msg) and matches[3] and matches[4] then
if  tonumber(matches[3]) < 1 or tonumber(matches[3]) > 120 then
		return "فقط بین 1 دقیقه تا 120 دقیقه مجاز به میوت همه پیام ها بصورت ساعتی میباشید"
end
local value = matches[4]
...
if matches[1] == "stats" and is_momod(msg) then
if matches[2] == "lock all" or matches[2] == "mute" then
local hash = "muteall:"..receiver
muteall = redis:pttl(hash)
local enable = redis:get(hash)
base = math.floor(muteall / 1000)
...
if matches[1] == "lock" and matches[2] == "all" and is_momod(msg) then
            local hash = "muteall:"..receiver
            local time = 3600 * 7
            redis:setex(hash, time, true)
			--savelog(msg.to.id, "پاک کننده همه پیام های گروه برای 7 ساعت فعال شد توسط "..msg.from.print_name:gsub("_",""))
...
--[[if matches[1] == "unlock" and matches[2] == "all" and is_momod(msg) then
    		local hash = "muteall:"..receiver
    		redis:del(hash)
			--savelog(msg.to.id, "پاک کننده همه پیام های گروه غیر فعال شد توسط "..msg.from.print_name:gsub("_",""))
            return "پاک کننده همه پیام ها غیر فعال گردید."
...
if type(msg.reply_id) ~= "nil" then
         function mute_reply(extra , success, result)
		  ...
		  return replymsg(extra.msg.reply_id, names.." در لیست افراد غیرمجاز در چت وجود دارد", ok_cb, false)
		  end
...
elseif matches[1] == "mute" and matches[2] and string.match(matches[2], "^%d+$") then
					...
					if is_muted_user(chat_id, matches[2]) then
		              return replymsg(msg.id, matches[2].." در لیست افراد غیرمجاز در چت وجود دارد", ok_cb, false)
		            end
					mute_user(chat_id, user_id)
...
elseif matches[1] == "mute" and matches[2] and not string.match(matches[2], "^%d+$") then
		...
		return false
		  ...
		  names = "@"..result.username
		  if is_muted_user(chat_id, user_id) then
		  return replymsg(extra.msg.id, names.." در لیست افراد غیرمجاز در چت وجود دارد", ok_cb, false)
		  end
		  mute_user(chat_id, user_id)
...
if type(msg.reply_id) ~= "nil" then
         function unmute_reply(extra , success, result)
		  ...
		  return replymsg(extra.msg.reply_id, names.." در لیست افراد غیرمجاز در چت وجود ندارد", ok_cb, false)
		  end
...
elseif matches[1] == "unmute"  and matches[2] and string.match(matches[2], "^%d+$") then
		  ...
		  return replymsg(msg.id, matches[2].." در لیست افراد غیرمجاز در چت وجود ندارد", ok_cb, false)
		  end
...
elseif matches[1] == "unmute" and matches[2] and not string.match(matches[2], "^%d+$") then
		...
		return false
		  ...
		  names = "@"..result.username
		  if not is_muted_user(extra.chat_id, user_id) then
		  return replymsg(extra.msg.id, names.." در لیست افراد غیرمجاز در چت وجود ندارد", ok_cb, false)
		  end
		  unmute_user(chat_id, user_id)
...
if matches[1] == "settings" and not matches[2] and is_momod(msg) then
   ...
   return replymsg(msg.id, settings(msg, chat_id), ok_cb, false)
end
--------------------------------------------------#rules and about
--------------------------------------------------#setabout
...
if matches[1] == "about" then
 if to_super then
    --savelog(msg.to.id, "اطلاعات گروه درخواست شد توسط "..msg.from.print_name:gsub("_"," "))
    return channel_info(receiver, callback_about, {receiver = receiver, msg = msg}) --replymsg(msg.id, "برای مشاهده توضیحات میتوانید پروفایل گروه را نگاه کنید", ok_cb, false)
 end
...
local function history(extra, suc, result)
  for i=1, #result do
    ...
    return sendmsg(receiver, "'"..extra.con.."' عدد پیام اخر پاک شدند", ok_cb, false)
  end
      local hash = "spamrmsg:"..msg.to.id..":"..msg.from.id
      local enable = redis:get(hash)
      if enable then
      return "<b>1 </b>دقیقه دیگر مجددا امتحان کنید"
      end
      if tonumber(matches[2]) > 100 or tonumber(matches[2]) < 10 then
        return "با عرض پوزش: عدد انتخابی باید بین 10 تا 100 باشد\nاین محدودیت از طرف <code>تلگرام </code> اعمال گردیده است\nهر 30 ثانیه این دستور را ارسال کنید"
      end
	  redis:setex(hash, 60, true)
	  --savelog(msg.to.id, "پیام های گروه پاک شدند توسط "..msg.from.print_name:gsub("_",""))
      get_history(receiver, matches[2] + 1 , history, {con = matches[2]})
end
...
if tonumber(i) > 99 then
    ...
    return sendmsg(receiver, "پیام ها پاک شدند")
end

if to_super then
      --savelog(msg.to.id, "پیام های گروه پاک شدند توسط "..msg.from.print_name:gsub("_",""))
      get_history(receiver, 500 + 1, history2, { con = 500})
end
end
--------------------------------------------------#id
if matches[1] == "id" then 
     if type(msg.reply_id)~="nil" and not matches[2] then
	    local function id_reply(extra , success, result)
		  local user_id = result.from.peer_id
          return replymsg(extra.msg.id, user_id)
        end
  ...
  elseif matches[1] == "id" and matches[2] and not string.match(matches[2], "^%d+$") then
    ...
    elseif matches[1] == "id" and not matches[2] then
       if to_chat or to_super then
	     ...
	     return replymsg(msg.id, "<b>"..msg.to.title.." </b>: <code>"..msg.to.id.." </code>")
		else
    ...
    elseif matches[1] == "kick" and matches[2] and string.match(matches[2], "^%d+$") then
         if tonumber(matches[2]) == tonumber(our_id) then
          ...
          if is_momod2(tonumber(matches[2]), msg.to.id) then
          	return send_larg_msg(receiver, "ایشان "..ranks2(tonumber(matches[2]), msg.to.id).." است", ok_cb, false)
         end
         if tonumber(matches[2]) == tonumber(msg.from.id) then
          	return
         end
...
if matches[1] == "kickme" then
   redis:set("kickme"..bot_divest..msg.from.id, true)
   redis:sadd("kickmes"..bot_divest, msg.from.id)
   return replymsg(msg.id, "برای اخراج مطمعن هستید ؟\nبا ارسال <b>yes </b> تا 30 ثانیه مهلت دارید اعلام موافقت نمایید", ok_cb, false)
end
--------------------------------------------------#ban
  if matches[1] == "ban" and is_momod(msg) then
    if type(msg.reply_id)~="nil" then
      function ban_reply(extra , success, result)
		  ...
		  return replymsg(extra.msg.id, " ایشان "..ranks2(user_id, chat_id).." است", ok_cb, false)
		  end
		  if is_banned(user_id, chat_id) then 
		  return replymsg(extra.msg.reply_id, names.." در لیست افراد بن وجود دارد", ok_cb, false)
		  end
    ...
    elseif matches[1] == "ban" and matches[2] and string.match(matches[2], "^%d+$") then
        ...
        if is_banned(matches[2], chat_id) then 
		  return replymsg(msg.id, matches[2].." در لیست افراد بن وجود دارد", ok_cb, false)
		  end
      ...
      elseif matches[1] == "ban" and matches[2] and not string.match(matches[2], "^%d+$") then
		...
		return false
		  ...
		  return replymsg(extra.msg.id, names.." در لیست افراد بن وجود دارد", ok_cb, false)
		  end
  ...
  if matches[1] == "unban" and is_momod(msg)  then
    if type(msg.reply_id)~="nil" then
      local function unban_reply(extra , success, result)
		  ...
		  return replymsg(extra.msg.reply_id, names.." در لیست افراد بن وجود ندارد", ok_cb, false)
		  end
    ...
    elseif matches[1] == "unban" and matches[2] and string.match(matches[2], "^%d+$") then
        if not is_banned(matches[2], chat_id) then 
		  return replymsg(msg.id, matches[2].." در لیست افراد بن وجود ندارد", ok_cb, false)
		  end
      ...
      elseif matches[1] == "unban" and matches[2] and not string.match(matches[2], "^%d+$") then
		...
		return false
		 ...
		 if not is_banned(user_id, extra.chat_id) then 
		  return replymsg(extra.msg.id, names.." در لیست افراد بن وجود ندارد", ok_cb, false)
		  end
   ...
   if not is_sudo(msg) then
    ...
    if type(msg.reply_id)~="nil" then
      local function banall_reply(extra , success, result)
		  ...
		  return replymsg(extra.msg.reply_id, names.." در لیست بن گلوبال وجود دارد", ok_cb, false)
		  end
          banall_user(user_id)
		  kick_user(user_id, group_id)
    ...
    elseif matches[1] == "banall" and matches[2] and string.match(matches[2], "^%d+$") then
        ...
        if is_gbanned(matches[2]) then
		  return replymsg(msg.id, matches[2].." در لیست بن گلوبال وجود دارد", ok_cb, false)
		end
      ...
      elseif matches[1] == "banall" and matches[2] and not string.match(matches[2], "^%d+$") then
		...
		return false
		 ...
		 local user_id = result.peer_id
		  ...
		  if is_gbanned(user_id) then
		  return replymsg(extra.msg.id, names.." در لیست بن گلوبال وجود دارد", ok_cb, false)
		  end
		 banall_user(user_id)
		 kick_user(user_id, extra.chat_id)
  ...
  if not is_sudo(msg) then
    ...
    if type(msg.reply_id)~="nil" then
      local function unbanall_reply(extra , success, result)
		  ...
		  return replymsg(extra.msg.reply_id, names.." در لیست بن گلوبال وجود ندارد", ok_cb, false)
		  end
    ...
    elseif matches[1] == "unbanall" and matches[2] and string.match(matches[2], "^%d+$") then
        if not is_gbanned(matches[2]) then
		  return replymsg(msg.id, matches[2].." در لیست بن گلوبال وجود ندارد", ok_cb, false)
		  end
      ...
      elseif matches[1] == "unbanall" and matches[2] and not string.match(matches[2], "^%d+$") then
		...
		return false
		 ...
		 local user_id = result.peer_id
		  if not is_gbanned(user_id) then
		  return replymsg(extra.msg.id, names.." در لیست بن گلوبال وجود ندارد", ok_cb, false)
		  end
		 unbanall_user(user_id)
...
--------------------------------------------------#unblock
  ...
  if not is_sudo(msg) then
    ...
    elseif matches[1] == "unblock" and matches[2] and string.match(matches[2], "^%d+$") then
        unblock_user("user#id"..matches[2], ok_cb, false)
		return  "[ "..matches[2].." ] از چت خصوصی ربات ان بلاک شد"
      elseif matches[1] == "unblock" and matches[2] and not string.match(matches[2], "^%d+$") then
...
if matches[1] == "addadmin" and is_sudo(msg) and matches[2] and string.match(matches[2], "^%d+$") then
 ...
 elseif matches[1] == "addadmin" and is_sudo(msg) and matches[2] and not string.match(matches[2], "^%d+$") then
		...
		return 	false
		 ...
		 if data[tostring(admins)][tostring(admin_id)] then
		  return replymsg(extra.msg.id, "@"..result.username.." is already an admin.", ok_cb, false)
	     end
           data[tostring(admins)][tostring(admin_id)] = "@"..result.username
	       save_data(data)
	       return replymsg(extra.msg.id, "@"..result.username.." has been promoted as admin.", ok_cb, false)
        end
		local username = string.gsub(matches[2], "@", "")
...
if matches[1] == "removeadmin" and is_sudo(msg) and matches[2] and string.match(matches[2], "^%d+$") then
 ...
 elseif matches[1] == "removeadmin" and is_sudo(msg) and matches[2] and not string.match(matches[2], "^%d+$") then
		...
		return false
		 ...
		 if not data[tostring(admins)][tostring(admin_id)] then
		  return replymsg(extra.msg.id, "@"..result.username.." is not an admin.", ok_cb, false)
	     end
           data[tostring(admins)][tostring(admin_id)] = nil
	       save_data(data)
	       return replymsg(extra.msg.id, "@"..result.username.." has been demoted as admin.", ok_cb, false)
        end
		local username = string.gsub(matches[2], "@", "")
...
--------------------------------------------------#banlist
   if matches[1] == "banlist" and is_momod(msg) then
    if matches[2] and is_admin1(msg) then
	 ...
	 --savelog(msg.to.id, "لیست افراد محروم از گروه درخواست شد توسط "..msg.from.print_name:gsub("_",""))
    return replymsg(msg.id, ban_list(chat_id), ok_cb, false)
  end
--------------------------------------------------#gbanlist
  if matches[1] == "gbanlist" and is_admin1(msg) then
    return replymsg(msg.id, banall_list(), ok_cb, false)
  end
--------------------------------------------------#gmutelist
  if matches[1] == "gmutelist" and is_admin1(msg) then
    return replymsg(msg.id, muteall_list(), ok_cb, false)
  end
--------------------------------------------------#kickedlist
  if matches[1] == "kicklist" and is_admin1(msg) then
    return replymsg(msg.id, kick_list(), ok_cb, false)
  end
--------------------------------------------------#divestlist
...
if matches[1] == "autoleave" and is_sudo(msg) then
     if matches[2] == "on" then
	    if redis:get("addgroup"..bot_divest) == "on" then
	    return "اتو لیو فعال است"
     else
	    redis:set("addgroup"..bot_divest, "on")
        return "اتو لیو فعال گردید"
	  end
    elseif matches[2] == "off" then
	    if redis:get("addgroup"..bot_divest) == "off" then
	    return "اتو لیو غیرفعال است"
    else
	   redis:set("addgroup"..bot_divest, "off")
...
--------------------------------------------------#bc
 if matches[1] == "bc" and is_admin1(msg) then
		...
		sendmsg("chat#id"..matches[2], response)
		sendmsg("channel#id"..matches[2], response)
		sendmsg("user#id"..matches[2], response)
 end
--------------------------------------------------#broadcast
 if matches[1] == "broadcast" and is_sudo(msg) then
			local i = 0
			for k,v in pairs(data[tostring("groups")]) do
				sendmsg("chat#id"..v, matches[2])
				sendmsg("channel#id"..v, matches[2])
				i = i + 1
		end
		       sendmsg(receiver, "به "..i.." گروه ارسال شد")
	end
--------------------------------------------------#all
...
if matches[1] == "chatlist" then
		if is_admin1(msg) and msg.to.type == "chat" or msg.to.type == "channel" then
			...
			sendmsg(receiver, "لیست گروه ها در چت خصوصی شما ارسال شد")
...
ویرایش شده https://github.com/yagop/telegram-bot تلگرام بوت

گروه منیجر و انتی اسپم نوشته شده به زبان لوا <b>Lua</b>

گرداورنده : telegram.me/valtman

کانال اختصاصی : telegram.me/ub_ch

برای نمایش قیمت ها nerkh را وارد کنید
]]
    return replymsg(msg.id, about, ok_cb, false)
  end
--------------------------------------------------#stats
...
if matches[1] == "facts" and is_admin1(msg) then
   get_message(msg.id, facts, {msg = msg})
    end
...
--------------------------------------------------#ping
  if matches[1] == "ping" then
    return replymsg(msg.id, [[<a href="telegram.me/ub_ch">PonG</a>]], ok_cb, false)
  end
--------------------------------------------------#command from normal users settings
   if matches[1] == "cmuser" and is_momod(msg) then
     if matches[2] == "lock" then
	    ...
	    return replymsg(msg.id, "محدودیت پاسخ ربات برای افراد عادی فعال است", ok_cb, false)
     else
        ...
        return replymsg(msg.id, "محدودیت پاسخ ربات برای افراد عادی فعال گردید", ok_cb, false)
	  end

    elseif matches[2] == "unlock" then
	    if redis:get("Groupcm"..bot_divest..":"..msg.to.id) == "off" then
	    return replymsg(msg.id, "محدودیت پاسخ ربات برای افراد عادی غیرفعال است", ok_cb, false)
    else
	    --savelog(msg.to.id, "محدودت پاسخ ربات برای افراد عادی گروه غیرفعال شد توسط "..msg.from.print_name:gsub("_",""))
	   redis:set("Groupcm"..bot_divest..":"..msg.to.id, "off")
       return replymsg(msg.id, "محدودیت پاسخ ربات برای افراد عادی غیرفعال گردید", ok_cb, false)
        end
     end
  end
--------------------------------------------------#type groups
...
--------------------------------------------------#list admins/groups
 if matches[1] == "list" and is_admin1 then 
	...
	elseif matches[2] == "admins" then
		return replymsg(msg.id, admin_list(msg), ok_cb, false)
	end
end
--------------------------------------------------#invite
 ...
 if type(msg.reply_id)~="nil" then
      local function invite_reply(extra , success, result)
	   ...
	   if is_banned(result.from.peer_id, extra.chat_id) and is_gbanned(result.from.peer_id) then 
        sendmsg(receiver, "User is banned and globaly banned")
	   elseif is_gbanned(result.from.peer_id) then
	    sendmsg(receiver, "User is globaly banned")
	   elseif is_banned(result.from.peer_id, extra.chat_id) then 
        sendmsg(receiver, "User is banned")
	   else
...
if matches[2] == "link" and matches[3] == "dis" then
	...
	return replymsg(msg.id, "پاک کننده لینک غیرفعال است", ok_cb, false)
	else
    data[tostring(target)]["settings"]["lock_link"] = "no"
    save_data(data)
    return replymsg(msg.id, "پاک کننده لینک غیرفعال گردید", ok_cb, false)
	end
	end
if matches[2] == "link" and matches[3] == "del" then
if data[tostring(target)]["settings"]["lock_link"] == "yes" then
	return replymsg(msg.id, "پاک کننده(پاک کردن) لینک فعال است", ok_cb, false)
	else
    data[tostring(target)]["settings"]["lock_link"] = "yes"
    save_data(data)
    return replymsg(msg.id, "پاک کننده(پاک کردن) لینک فعال گردید", ok_cb, false)
	end
	end
if matches[2] == "link" and matches[3] == "kick" then
if data[tostring(target)]["settings"]["lock_link"] == "kick" then
	return replymsg(msg.id, "پاک کننده(ریمو کردن) لینک فعال است", ok_cb, false)
	else
    data[tostring(target)]["settings"]["lock_link"] = "kick"
    save_data(data)
    return replymsg(msg.id, "پاک کننده(ریمو کردن) لینک فعال گردید", ok_cb, false)
end
end

if matches[2] == "spam" and matches[3] == "dis" then
if not is_owner(msg) then
return replymsg(msg.id, "برای تنظیم این قفل به مقام صاحب گروه(owner) نیاز است", ok_cb, false)
end
    data[tostring(target)]["settings"]["lock_spam"] = "no"
    save_data(data)
    return replymsg(msg.id, "پاک کننده پیام های طولانی غیر فعال گردید", ok_cb, false)
	end
...
return replymsg(msg.id, "برای تنظیم این قفل به مقام صاحب گروه(owner) نیاز است", ok_cb, false)
    ...
    data[tostring(target)]["settings"]["lock_spam"] = "yes"
    save_data(data)
    return replymsg(msg.id, "پاک کننده(پاک کردن) پیام های طولانی فعال گردید", ok_cb, false)
	end
...
return replymsg(msg.id, "برای تنظیم این قفل به مقام صاحب گروه(owner) نیاز است", ok_cb, false)
    ...
    save_data(data)
    return replymsg(msg.id, "پاک کننده(ریمو کردن) پیام های طولانی فعال گردید", ok_cb, false)
end

if matches[2] == "flood" and matches[3] == "dis" then
if not is_admin1(msg) then
return replymsg(msg.id, "برای تنظیم این قفل به مقام ادمین ربات نیاز است", ok_cb, false)
end
    data[tostring(target)]["settings"]["lock_flood"] = "no"
    save_data(data)
    return replymsg(msg.id, "پاک کننده پیام های مکرر غیر فعال گردید", ok_cb, false)
	end
...
return replymsg(msg.id, "برای تنظیم این قفل به مقام ادمین ربات نیاز است", ok_cb, false)
    ...
    data[tostring(target)]["settings"]["lock_flood"] = "yes"
    save_data(data)
    return replymsg(msg.id, "پاک کننده(پاک کردن) پیام های مکرر فعال گردید", ok_cb, false)
	end
...
return replymsg(msg.id, "برای تنظیم این قفل به مقام ادمین ربات نیاز است", ok_cb, false)
    ...
    data[tostring(target)]["settings"]["lock_flood"] = "yes"
    save_data(data)
    return replymsg(msg.id, "پاک کننده(ریمو کردن) پیام مکرر فعال گردید", ok_cb, false)
end

if matches[2] == "fa" and matches[3] == "dis" then
    data[tostring(target)]["settings"]["lock_arabic"] = "no"
    save_data(data)
    return replymsg(msg.id, "پاک کننده پیام های فارسی غیر فعال گردید", ok_cb, false)
	end
if matches[2] == "fa" and matches[3] == "del" then
    data[tostring(target)]["settings"]["lock_arabic"] = "yes"
    save_data(data)
    return replymsg(msg.id, "پاک کننده(پاک کردن) پیام های فارسی فعال گردید", ok_cb, false)
	end
if matches[2] == "fa" and matches[3] == "kick" then
    data[tostring(target)]["settings"]["lock_arabic"] = "kick"
    save_data(data)
    return replymsg(msg.id, "پاک کننده(ریمو کردن) پیام های فارسی فعال گردید", ok_cb, false)
end

if matches[2] == "en" and matches[3] == "dis" then
    data[tostring(target)]["settings"]["lock_en"] = "no"
    save_data(data)
    return replymsg(msg.id, "پاک کننده پیام های انگلیسی غیر فعال گردید", ok_cb, false)
	end
if matches[2] == "en" and matches[3] == "del" then
    data[tostring(target)]["settings"]["lock_en"] = "yes"
    save_data(data)
    return replymsg(msg.id, "پاک کننده(پاک کردن) پیام های انگلیسی فعال گردید", ok_cb, false)
	end
if matches[2] == "en" and matches[3] == "kick" then
    data[tostring(target)]["settings"]["lock_en"] = "kick"
    save_data(data)
    return replymsg(msg.id, "پاک کننده(ریمو کردن) پیام های انگلیسی فعال گردید", ok_cb, false)
end

if matches[2] == "member" and matches[3] == "dis" then
if to_super then
return replymsg(msg.id, "این عملکرد در سوپر گروه ها مجاز نیست", ok_cb, false)
end
    data[tostring(target)]["settings"]["lock_member"] = "no"
    save_data(data)
    return replymsg(msg.id, "پاک کننده افراد دعوت شده غیر فعال گردید", ok_cb, false)
	end
...
return replymsg(msg.id, "این عملکرد در سوپر گروه ها مجاز نیست", ok_cb, false)
    ...
    save_data(data)
    return replymsg(msg.id, "پاک کننده افراد دعوت شده فعال گردید(فقط دعوت شده ریمو میشود)", ok_cb, false)
	end
...
return replymsg(msg.id, "این عملکرد در سوپر گروه ها مجاز نیست", ok_cb, false)
    ...
    save_data(data)
    return replymsg(msg.id, "پاک کننده افراد دعوت شده فعال گردید(دعوت کننده و دعوت شده ریمو میشوند)", ok_cb, false)
end

if matches[2] == "tgservice" and matches[3] == "dis" then
    data[tostring(target)]["settings"]["lock_tgservice"] = "no"
    save_data(data)
    return replymsg(msg.id, "پاک کننده پیام های جوین،دعوت،ریمو،عوض کردن اسم و... غیر فعال گردید", ok_cb, false)
	end
if matches[2] == "tgservice" and matches[3] == "del" then
    data[tostring(target)]["settings"]["lock_tgservice"] = "yes"
    save_data(data)
    return replymsg(msg.id, "پاک کننده(پاک کردن) پیام های جوین،دعوت،ریمو،عوض کردن اسم و... فعال گردید", ok_cb, false)
	end
if matches[2] == "tgservice" and matches[3] == "kick" then
    data[tostring(target)]["settings"]["lock_tgservice"] = "yes"
    save_data(data)
    return replymsg(msg.id, "[امکان ریمو نیست]پاک کننده(پاک کردن) پیام های جوین،دعوت،ریمو،عوض کردن اسم و... فعال گردید", ok_cb, false)
end

if matches[2] == "sticker" and matches[3] == "dis" then
    data[tostring(target)]["settings"]["lock_sticker"] = "no"
    save_data(data)
    return replymsg(msg.id, "پاک کننده استیکر غیر فعال گردید", ok_cb, false)
	end
if matches[2] == "sticker" and matches[3] == "del" then
    data[tostring(target)]["settings"]["lock_sticker"] = "yes"
    save_data(data)
    return replymsg(msg.id, "پاک کننده(پاک کردن) استیکر فعال گردید", ok_cb, false)
	end
if matches[2] == "sticker" and matches[3] == "kick" then
    data[tostring(target)]["settings"]["lock_sticker"] = "kick"
    save_data(data)
    return replymsg(msg.id, "پاک کننده(ریمو کردن) استیکر فعال گردید", ok_cb, false)
end

if matches[2] == "share" and matches[3] == "dis" then
    data[tostring(target)]["settings"]["lock_contact"] = "no"
    save_data(data)
    return replymsg(msg.id, "پاک کننده شماره غیر فعال گردید", ok_cb, false)
	end
if matches[2] == "share" and matches[3] == "del" then
    data[tostring(target)]["settings"]["lock_contact"] = "yes"
    save_data(data)
    return replymsg(msg.id, "پاک کننده(پاک کردن) شماره فعال گردید", ok_cb, false)
	end
if matches[2] == "share" and matches[3] == "kick" then
    data[tostring(target)]["settings"]["lock_contact"] = "kick"
    save_data(data)
    return replymsg(msg.id, "پاک کننده(ریمو کردن) شماره فعال گردید", ok_cb, false)
end

if matches[2] == "photo" and matches[3] == "dis" then
    data[tostring(target)]["settings"]["lock_photo"] = "no"
    save_data(data)
    return replymsg(msg.id, "پاک کننده عکس غیر فعال گردید", ok_cb, false)
	end
if matches[2] == "photo" and matches[3] == "del" then
    data[tostring(target)]["settings"]["lock_photo"] = "yes"
    save_data(data)
    return replymsg(msg.id, "پاک کننده(پاک کردن) عکس فعال گردید", ok_cb, false)
	end
if matches[2] == "photo" and matches[3] == "kick" then
    data[tostring(target)]["settings"]["lock_photo"] = "kick"
    save_data(data)
    return replymsg(msg.id, "پاک کننده(ریمو کردن) عکس فعال گردید", ok_cb, false)
end

if matches[2] == "text" and matches[3] == "dis" then
    data[tostring(target)]["settings"]["lock_text"] = "no"
    save_data(data)
    return replymsg(msg.id, "پاک کننده چت(متن) غیر فعال گردید", ok_cb, false)
	end
if matches[2] == "text" and matches[3] == "del" then
    data[tostring(target)]["settings"]["lock_text"] = "yes"
    save_data(data)
    return replymsg(msg.id, "پاک کننده(پاک کردن) چت(متن) فعال گردید", ok_cb, false)
	end
if matches[2] == "text" and matches[3] == "kick" then
    data[tostring(target)]["settings"]["lock_text"] = "kick"
    save_data(data)
    return replymsg(msg.id, "پاک کننده(ریمو کردن) چت(متن) فعال گردید", ok_cb, false)
end

if matches[2] == "audio" and matches[3] == "dis" then
    data[tostring(target)]["settings"]["lock_audio"] = "no"
    save_data(data)
    return replymsg(msg.id, "پاک کننده صدا غیر فعال گردید", ok_cb, false)
	end
if matches[2] == "audio" and matches[3] == "del" then
    data[tostring(target)]["settings"]["lock_audio"] = "yes"
    save_data(data)
    return replymsg(msg.id, "پاک کننده(پاک کردن) صدا فعال گردید", ok_cb, false)
	end
if matches[2] == "audio" and matches[3] == "kick" then
    data[tostring(target)]["settings"]["lock_audio"] = "kick"
    save_data(data)
    return replymsg(msg.id, "پاک کننده(ریمو کردن) صدا فعال گردید", ok_cb, false)
end

if matches[2] == "video" and matches[3] == "dis" then
    data[tostring(target)]["settings"]["lock_video"] = "no"
    save_data(data)
    return replymsg(msg.id, "پاک کننده فیلم غیر فعال گردید", ok_cb, false)
	end
if matches[2] == "video" and matches[3] == "del" then
    data[tostring(target)]["settings"]["lock_video"] = "yes"
    save_data(data)
    return replymsg(msg.id, "پاک کننده(پاک کردن) فیلم فعال گردید", ok_cb, false)
	end
if matches[2] == "video" and matches[3] == "kick" then
    data[tostring(target)]["settings"]["lock_video"] = "kick"
    save_data(data)
    return replymsg(msg.id, "پاک کننده(ریمو کردن) فیلم فعال گردید", ok_cb, false)
end

if matches[2] == "file" and matches[3] == "dis" then
    data[tostring(target)]["settings"]["lock_document"] = "no"
    save_data(data)
    return replymsg(msg.id, "پاک کننده فایل غیر فعال گردید", ok_cb, false)
	end
if matches[2] == "file" and matches[3] == "del" then
    data[tostring(target)]["settings"]["lock_document"] = "yes"
    save_data(data)
    return replymsg(msg.id, "پاک کننده(پاک کردن) فایل فعال گردید", ok_cb, false)
	end
if matches[2] == "file" and matches[3] == "kick" then
    data[tostring(target)]["settings"]["lock_document"] = "kick"
    save_data(data)
    return replymsg(msg.id, "پاک کننده(ریمو کردن) فایل فعال گردید", ok_cb, false)
end

if matches[2] == "gif" and matches[3] == "dis" then
    data[tostring(target)]["settings"]["lock_gif"] = "no"
    save_data(data)
    return replymsg(msg.id, "پاک کننده گیف(عکس متحرک) غیر فعال گردید", ok_cb, false)
	end
if matches[2] == "gif" and matches[3] == "del" then
    data[tostring(target)]["settings"]["lock_gif"] = "yes"
    save_data(data)
    return replymsg(msg.id, "پاک کننده(پاک کردن) گیف(عکس متحرک) فعال گردید", ok_cb, false)
	end
if matches[2] == "gif" and matches[3] == "kick" then
    data[tostring(target)]["settings"]["lock_gif"] = "kick"
    save_data(data)
    return replymsg(msg.id, "پاک کننده(ریمو کردن) گیف(عکس متحرک) فعال گردید", ok_cb, false)
end

if matches[2] == "fwd" and matches[3] == "dis" then
    data[tostring(target)]["settings"]["lock_fwd"] = "no"
    save_data(data)
    return replymsg(msg.id, "پاک کننده پیام های فوروارد غیر فعال گردید", ok_cb, false)
	end
if matches[2] == "fwd" and matches[3] == "del" then
    data[tostring(target)]["settings"]["lock_fwd"] = "yes"
    save_data(data)
    return replymsg(msg.id, "پاک کننده(پاک کردن) پیام های فوروارد فعال گردید", ok_cb, false)
	end
if matches[2] == "fwd" and matches[3] == "kick" then
    data[tostring(target)]["settings"]["lock_fwd"] = "kick"
    save_data(data)
    return replymsg(msg.id, "پاک کننده(ریمو کردن) پیام های فوروارد فعال گردید", ok_cb, false)
end

if matches[2] == "bot" and matches[3] == "dis" then
    data[tostring(target)]["settings"]["lock_bot"] = "no"
    save_data(data)
    return replymsg(msg.id, "ضد ربات غیر فعال گردید", ok_cb, false)
	end
if matches[2] == "bot" and matches[3] == "inv" then
    data[tostring(target)]["settings"]["lock_bot"] = "kickinv"
    save_data(data)
    return replymsg(msg.id, "ضد ربات فعال شد(فقط ربات ریمو میشود)", ok_cb, false)
	end
if matches[2] == "bot" and matches[3] == "both" then
    data[tostring(target)]["settings"]["lock_bot"] = "kickboth"
    save_data(data)
    return replymsg(msg.id, "ضد ربات فعال شد(هم ربات و هم دعوت کننده ریمو میشوند)", ok_cb, false)
end
...
return replymsg(msg.id, "برای تنظیم این قفل به مقام صاحب گروه(owner) نیاز است", ok_cb, false)
    ...
    data[tostring(target)]["settings"]["lock_join"] = "no"
    save_data(data)
    return replymsg(msg.id, "پاک کننده افراد جوین دهنده با لینک غیر فعال گردید", ok_cb, false)
	end
...
return replymsg(msg.id, "برای تنظیم این قفل به مقام صاحب گروه(owner) نیاز است", ok_cb, false)
    ...
    save_data(data)
    return replymsg(msg.id, "پاک کننده(ریمو کردن) افراد جوین دهنده با لینک فعال گردید[امکان پاک کردن نیست]", ok_cb, false)
	end
...
return replymsg(msg.id, "برای تنظیم این قفل به مقام صاحب گروه(owner) نیاز است", ok_cb, false)
    ...
    data[tostring(target)]["settings"]["lock_join"] = "kick"
    save_data(data)
    return replymsg(msg.id, "پاک کننده(ریمو کردن) افراد جوین دهنده با لینک فعال گردید", ok_cb, false)
end

if matches[2] == "reply" and matches[3] == "dis" then
    data[tostring(target)]["settings"]["lock_reply"] = "no"
    save_data(data)
    return replymsg(msg.id, "پاک کننده پیام های ریپلی(پاسخ) غیرفعال گردید", ok_cb, false)
	end
if matches[2] == "reply" and matches[3] == "del" then
    data[tostring(target)]["settings"]["lock_reply"] = "yes"
    save_data(data)
    return replymsg(msg.id, "پاک کننده(پاک کردن) پیام های ریپلی(پاسخ) فعال گردید", ok_cb, false)
	end
if matches[2] == "reply" and matches[3] == "kick" then
    data[tostring(target)]["settings"]["lock_reply"] = "kick"
    save_data(data)
    return replymsg(msg.id, "پاک کننده(ریمو کردن) پیام های ریپلی(پاسخ) فعال گردید", ok_cb, false)
end

if matches[2] == "tag" and matches[3] == "dis" then
    data[tostring(target)]["settings"]["lock_tag"] = "no"
    save_data(data)
    return replymsg(msg.id, "پاک کننده پیام های دارای تگ غیر فعال گردید", ok_cb, false)
	end
if matches[2] == "tag" and matches[3] == "del" then
    data[tostring(target)]["settings"]["lock_tag"] = "yes"
    save_data(data)
    return replymsg(msg.id, "پاک کننده(پاک کردن) پیام های دارای تگ فعال گردید", ok_cb, false)
	end
if matches[2] == "tag" and matches[3] == "kick" then
    data[tostring(target)]["settings"]["lock_tag"] = "kick"
    save_data(data)
    return replymsg(msg.id, "پاک کننده(ریمو کردن) پیام های دارای تگ فعال گردید", ok_cb, false)
end
if matches[2] == "porn" and matches[3] == "dis" then
    data[tostring(target)]["settings"]["lock_porn"] = "no"
    save_data(data)
    return replymsg(msg.id, "پاک کننده تصاویر +18 غیر فعال گردید", ok_cb, false)
	end
if matches[2] == "porn" and matches[3] == "del" then
    data[tostring(target)]["settings"]["lock_porn"] = "yes"
    save_data(data)
    return replymsg(msg.id, "پاک کننده(پاک کردن) تصاویر +18 فعال گردید", ok_cb, false)
	end
if matches[2] == "porn" and matches[3] == "kick" then
    data[tostring(target)]["settings"]["lock_porn"] = "kick"
    save_data(data)
    return replymsg(msg.id, "پاک کننده(ریمو کردن) تصاویر +18 فعال گردید", ok_cb, false)
end
end
...
if data[tostring(target)]["settings"]["lock_spam"] == "no" then
	return replymsg(msg.id, "پاک کننده پیام های طولانی غیرفعال است", ok_cb, false)
	else
    data[tostring(target)]["settings"]["lock_spam"] = "no"
    save_data(data)
	--savelog(msg.to.id, "پاک کننده پیام های طولانی غیرفعال گردید توسط "..msg.from.print_name:gsub("_",""))
    return replymsg(msg.id, "پاک کننده پیام های طولانی غیرفعال گردید", ok_cb, false)
end

elseif matches[2] == "flood" then
 if data[tostring(target)]["settings"]["lock_flood"] == "no" then
	return replymsg(msg.id, "پاک کننده پیامهای مکرر غیرفعال است", ok_cb, false)
	else
    data[tostring(target)]["settings"]["lock_flood"] = "no"
    save_data(data)
	--savelog(msg.to.id, "پاک کننده پیامهای مکرر غیرفعال گردید توسط "..msg.from.print_name:gsub("_",""))
    return replymsg(msg.id, "پاک کننده پیامهای مکرر غیرفعال گردید", ok_cb, false)
end

elseif matches[2] == "fa" then
if data[tostring(target)]["settings"]["lock_arabic"] == "no" then
	return replymsg(msg.id, "پاک کننده پیام های فارسی غیر فعال است", ok_cb, false)
	else
    data[tostring(target)]["settings"]["lock_arabic"] = "no"
    save_data(data)
...
if data[tostring(target)]["settings"]["lock_en"] == "no" then
	return replymsg(msg.id, "پاک کننده پیام های انگلیسی غیر فعال است", ok_cb, false)
	else
    data[tostring(target)]["settings"]["lock_en"] = "no"
    save_data(data)
...
if data[tostring(target)]["settings"]["lock_member"] == "no" then
	return replymsg(msg.id, "پاک کننده افراد دعوت شده غیر فعال است", ok_cb, false)
	else
    data[tostring(target)]["settings"]["lock_member"] = "no"
    save_data(data)
...
if data[tostring(target)]["settings"]["lock_tgservice"] == "no" then
	return replymsg(msg.id, "پاک کننده پیام های جوین،دعوت،ریمو،عوض کردن اسم و... غیر فعال است", ok_cb, false)
	else
    data[tostring(target)]["settings"]["lock_tgservice"] = "no"
    save_data(data)
...
if data[tostring(target)]["settings"]["lock_sticker"] == "no" then
	return replymsg(msg.id, "پاک کننده استیکر غیر فعال است", ok_cb, false)
	else
    data[tostring(target)]["settings"]["lock_sticker"] = "no"
...
if data[tostring(target)]["settings"]["lock_contact"] == "no" then
	return replymsg(msg.id, "پاک کننده شماره غیر فعال است", ok_cb, false)
	else
    data[tostring(target)]["settings"]["lock_contact"] = "no"
    save_data(data)
...
if data[tostring(target)]["settings"]["lock_photo"] == "no" then
	return replymsg(msg.id, "پاک کننده عکس غیر فعال است", ok_cb, false)
	else
    data[tostring(target)]["settings"]["lock_photo"] = "no"
    save_data(data)
...
if data[tostring(target)]["settings"]["lock_text"] == "no" then
	return replymsg(msg.id, "پاک کننده چت(متن) غیر فعال است", ok_cb, false)
	else
    data[tostring(target)]["settings"]["lock_text"] = "no"
    save_data(data)
...
if data[tostring(target)]["settings"]["lock_audio"] == "no" then
	return replymsg(msg.id, "پاک کننده صدا غیر فعال است", ok_cb, false)
	else
    data[tostring(target)]["settings"]["lock_audio"] = "no"
    save_data(data)
...
if data[tostring(target)]["settings"]["lock_video"] == "no" then
	return replymsg(msg.id, "پاک کننده فیلم غیر فعال است", ok_cb, false)
	else
    data[tostring(target)]["settings"]["lock_video"] = "no"
    save_data(data)
...
if data[tostring(target)]["settings"]["lock_document"] == "no" then
	return replymsg(msg.id, "پاک کننده فایل غیر فعال است", ok_cb, false)
	else
    data[tostring(target)]["settings"]["lock_document"] = "no"
    save_data(data)
...
if data[tostring(target)]["settings"]["lock_gif"] == "no" then
	return replymsg(msg.id, "پاک کننده گیف(عکس متحرک) غیر فعال است", ok_cb, false)
	else
    data[tostring(target)]["settings"]["lock_gif"] = "no"
    save_data(data)
...
if data[tostring(target)]["settings"]["lock_fwd"] == "no" then
	return replymsg(msg.id, "پاک کننده پیام های فوروارد غیر فعال است", ok_cb, false)
	else
    data[tostring(target)]["settings"]["lock_fwd"] = "no"
    save_data(data)
...
if data[tostring(target)]["settings"]["lock_bot"] == "no" then
	return replymsg(msg.id, "ضد ربات غیر فعال است", ok_cb, false)
	else
    data[tostring(target)]["settings"]["lock_bot"] = "no"
    save_data(data)
...
if data[tostring(target)]["settings"]["lock_join"] == "no" then
	return replymsg(msg.id, "پاک کننده افراد جوین دهنده با لینک غیر فعال است", ok_cb, false)
	else
    data[tostring(target)]["settings"]["lock_join"] = "no"
    save_data(data)
...
if data[tostring(target)]["settings"]["lock_reply"] == "no" then
	return replymsg(msg.id, "پاک کننده پیام های ریپلی(پاسخ) غیرفعال است", ok_cb, false)
	else
    data[tostring(target)]["settings"]["lock_reply"] = "no"
    save_data(data)
...
if data[tostring(target)]["settings"]["lock_tag"] == "no" then
	return replymsg(msg.id, "پاک کننده پیام های دارای تگ غیر فعال است", ok_cb, false)
	else
    data[tostring(target)]["settings"]["lock_tag"] = "no"
    save_data(data)
...
if data[tostring(target)]["settings"]["lock_unsup"] == "no" then
	return replymsg(msg.id, "پاک کننده پیام های ناشناخته غیرفعال است", ok_cb, false)
	else
    data[tostring(target)]["settings"]["lock_unsup"] = "no"
    save_data(data)
	--savelog(msg.to.id, "پاک کننده  پیام های ناشناخته غیرفعال گردید توسط "..msg.from.print_name:gsub("_",""))
    return replymsg(msg.id, "پاک کننده پیام های ناشناخته غیر فعال گردید", ok_cb, false)
end

elseif matches[2] == "media" then
if data[tostring(target)]["settings"]["lock_media"] == "no" then
	return replymsg(msg.id, "پاک کننده مدیا(هرچیزی غیر از متن) غیرفعال است", ok_cb, false)
	else
    data[tostring(target)]["settings"]["lock_media"] = "no"
    save_data(data)
	--savelog(msg.to.id, "پاک کننده مدیا(هرچیزی غیر از متن) غیرفعال گردید توسط "..msg.from.print_name:gsub("_",""))
    return replymsg(msg.id, "پاک کننده مدیا(هرچیزی غیر از متن) غیرفعال گردید", ok_cb, false)
end

elseif matches[2] == "caption" then
if data[tostring(target)]["settings"]["lock_capmedia"] == "no" then
	return replymsg(msg.id, "پاک کننده پیام های دارای عنوان(کپشن) غیرفعال است", ok_cb, false)
	else
    data[tostring(target)]["settings"]["lock_capmedia"] = "no"
    save_data(data)
	--savelog(msg.to.id, "پاک کننده پیام های دارای عنوان(کپشن) غیرفعال گردید توسط "..msg.from.print_name:gsub("_",""))
    return replymsg(msg.id, "پاک کننده پیام های دارای عنوان(کپشن) غیرفعال گردید", ok_cb, false)
end

elseif matches[2] == "porn" then
if data[tostring(target)]["settings"]["lock_porn"] == "no" then
	return replymsg(msg.id, "پاک کننده تصاویر +18 غیرفعال است", ok_cb, false)
	else
    data[tostring(target)]["settings"]["lock_porn"] = "no"
    save_data(data)
	--savelog(msg.to.id, "پاک کننده پیام های دارای عنوان(کپشن) غیرفعال گردید توسط "..msg.from.print_name:gsub("_",""))
    return replymsg(msg.id, "پاک کننده تصاویر +18 غیرفعال گردید", ok_cb, false)
end
...
return replymsg(msg.id, "جناب با عرض پوزش شما در هر (<code>30</code>) دقیقه قادر به انلاک کردن همه قفل ها هستید\n(<b>"..base.."</b>) دقیقه دیگه امتحان کنید", ok_cb, false)
end
redis:setex(hash, time, true)
local target = msg.to.id
if data[tostring(target)]["settings"] then
...
redis:del(hash)
    ...
    return replymsg(msg.id, "همه ی پاک کننده ها غیر فعال شدند", ok_cb, false)
      end
   end
end
--------------------------------------------------#spam limit
...
return "عدد ورودی باید بین 500 تا 4000 باشد در غیر اینصورت از /UNLOCK SPAM استفاده کنید"
    ...
    return replymsg(msg.id, "محدودیت پاک کننده پیام های طولانی محدود شد به "..matches[2], ok_cb, false)
end
...
if matches[2] == "link" then
 if data[tostring(target)]["settings"]["lock_link"] == "yes" then
	return replymsg(msg.id, "پاک کننده لینک فعال است", ok_cb, false)
	else
    data[tostring(target)]["settings"]["lock_link"] = "yes"
    save_data(data)
	--savelog(msg.to.id, "پاک کننده لینک فعال گردید توسط "..msg.from.print_name:gsub("_",""))
    return replymsg(msg.id, "پاک کننده لینک فعال گردید", ok_cb, false)
end

elseif matches[2] == "spam" then
if data[tostring(target)]["settings"]["lock_spam"] == "yes" then
	return replymsg(msg.id, "پاک کننده پیام های طولانی فعال است", ok_cb, false)
	else
    data[tostring(target)]["settings"]["lock_spam"] = "yes"
    save_data(data)
	--savelog(msg.to.id, "پاک کننده پیام های طولانی فعال گردید توسط "..msg.from.print_name:gsub("_",""))
    return replymsg(msg.id, "پاک کننده پیام های طولانی فعال گردید", ok_cb, false)
end

elseif matches[2] == "flood" then
if data[tostring(target)]["settings"]["lock_flood"] == "yes" then
	return replymsg(msg.id, "پاک کننده پیامهای مکرر فعال است", ok_cb, false)
	else
    data[tostring(target)]["settings"]["lock_flood"] = "yes"
    save_data(data)
	--savelog(msg.to.id, "پاک کننده  پیامهای مکرر فعال گردید توسط "..msg.from.print_name:gsub("_",""))
    return replymsg(msg.id, "پاک کننده پیامهای مکرر فعال گردید", ok_cb, false)
end

elseif matches[2] == "fa" then
if data[tostring(target)]["settings"]["lock_arabic"] == "yes" then
	return replymsg(msg.id, "پاک کننده پیام های فارسی فعال است", ok_cb, false)
	else
    data[tostring(target)]["settings"]["lock_arabic"] = "yes"
    save_data(data)
	--savelog(msg.to.id, "پاک کننده پیام های فارسی فعال گردید توسط "..msg.from.print_name:gsub("_",""))
    return replymsg(msg.id, "پاک کننده پیام های فارسی فعال گردید", ok_cb, false)
end

elseif matches[2] == "en" then
if data[tostring(target)]["settings"]["lock_en"] == "yes" then
	return replymsg(msg.id, "پاک کننده پیام های انگلیسی فعال است", ok_cb, false)
	else
    data[tostring(target)]["settings"]["lock_en"] = "yes"
    save_data(data)
	--savelog(msg.to.id, "پاک کننده پیام های انگلیسی فعال گردید توسط "..msg.from.print_name:gsub("_",""))
    return replymsg(msg.id, "پاک کننده پیام های انگلیسی فعال گردید", ok_cb, false)
end
...
if data[tostring(target)]["settings"]["lock_member"] == "yes" then
	return replymsg(msg.id, "پاک کننده افراد دعوت شده فعال است", ok_cb, false)
	else
    data[tostring(target)]["settings"]["lock_member"] = "yes"
    save_data(data)
	--savelog(msg.to.id, "پاک کننده افراد دعوت شده فعال گردید توسط "..msg.from.print_name:gsub("_",""))
    return replymsg(msg.id, "پاک کننده افراد دعوت شده فعال گردید", ok_cb, false)
end

elseif matches[2] == "tgservice" then
if data[tostring(target)]["settings"]["lock_tgservice"] == "yes" then
	return replymsg(msg.id, "پاک کننده پیام های جوین،دعوت،ریمو،عوض کردن اسم و... فعال است", ok_cb, false)
	else
    data[tostring(target)]["settings"]["lock_tgservice"] = "yes"
    save_data(data)
	--savelog(msg.to.id, "پاک کننده پیام های جوین،دعوت،ریمو،عوض کردن اسم و... فعال گردید توسط "..msg.from.print_name:gsub("_",""))
    return replymsg(msg.id, "پاک کننده پیام های جوین،دعوت،ریمو،عوض کردن اسم و... فعال گردید", ok_cb, false)
end

elseif matches[2] == "sticker" then
if data[tostring(target)]["settings"]["lock_capmedia"] == "yes" then
	return replymsg(msg.id, "پاک کننده پیام های دارای عنوان(کپشن) فعال است", ok_cb, false)
end
if data[tostring(target)]["settings"]["lock_sticker"] == "yes" then
	return replymsg(msg.id, "پاک کننده استیکر فعال است", ok_cb, false)
	else
    data[tostring(target)]["settings"]["lock_sticker"] = "yes"
    save_data(data)
	--savelog(msg.to.id, "پاک کننده استیکر فعال گردید توسط "..msg.from.print_name:gsub("_",""))
    return replymsg(msg.id, "پاک کننده استیکر فعال گردید", ok_cb, false)
end
...
if data[tostring(target)]["settings"]["lock_contact"] == "yes" then
	return replymsg(msg.id, "پاک کننده شماره فعال است", ok_cb, false)
	else
    data[tostring(target)]["settings"]["lock_contact"] = "yes"
    save_data(data)
	--savelog(msg.to.id, "پاک کننده شماره فعال گردید توسط "..msg.from.print_name:gsub("_",""))
    return replymsg(msg.id, "پاک کننده شماره فعال گردید", ok_cb, false)
end
...
if data[tostring(target)]["settings"]["lock_photo"] == "yes" then
	return replymsg(msg.id, "پاک کننده عکس فعال است", ok_cb, false)
	else
    data[tostring(target)]["settings"]["lock_photo"] = "yes"
    save_data(data)
	--savelog(msg.to.id, "پاک کننده عکس فعال گردید توسط "..msg.from.print_name:gsub("_",""))
    return replymsg(msg.id, "پاک کننده عکس فعال گردید", ok_cb, false)
end
...
if data[tostring(target)]["settings"]["lock_text"] == "yes" then
	return replymsg(msg.id, "پاک کننده چت(متن) فعال است", ok_cb, false)
	else
    data[tostring(target)]["settings"]["lock_text"] = "yes"
    save_data(data)
	--savelog(msg.to.id, "پاک کننده چت(متن) فعال گردید توسط "..msg.from.print_name:gsub("_",""))
    return replymsg(msg.id, "پاک کننده چت(متن) فعال گردید", ok_cb, false)
end
...
if data[tostring(target)]["settings"]["lock_audio"] == "yes" then
	return replymsg(msg.id, "پاک کننده صدا فعال است", ok_cb, false) 
	else
    data[tostring(target)]["settings"]["lock_audio"] = "yes"
    save_data(data)
	--savelog(msg.to.id, "پاک کننده صدا فعال گردید توسط "..msg.from.print_name:gsub("_",""))
    return replymsg(msg.id, "پاک کننده صدا فعال گردید", ok_cb, false) 
end
...
if data[tostring(target)]["settings"]["lock_video"] == "yes" then
	return replymsg(msg.id, "پاک کننده فیلم فعال است", ok_cb, false) 
	else
    data[tostring(target)]["settings"]["lock_video"] = "yes"
    save_data(data)
	--savelog(msg.to.id, "پاک کننده فیلم فعال گردید توسط "..msg.from.print_name:gsub("_",""))
    return replymsg(msg.id, "پاک کننده فیلم فعال گردید", ok_cb, false) 
end
...
if data[tostring(target)]["settings"]["lock_document"] == "yes" then
	return replymsg(msg.id, "پاک کننده فایل فعال است", ok_cb, false) 
	else
    data[tostring(target)]["settings"]["lock_document"] = "yes"
    save_data(data)
	--savelog(msg.to.id, "پاک کننده فایل فعال گردید توسط "..msg.from.print_name:gsub("_",""))
    return replymsg(msg.id, "پاک کننده فایل فعال گردید", ok_cb, false) 
end
...
if data[tostring(target)]["settings"]["lock_gif"] == "yes" then
	return replymsg(msg.id, "پاک کننده گیف(عکس متحرک) فعال است", ok_cb, false)
	else
    data[tostring(target)]["settings"]["lock_gif"] = "yes"
    save_data(data)
	--savelog(msg.to.id, "پاک کننده گیف(عکس متحرک) فعال گردید توسط "..msg.from.print_name:gsub("_",""))
    return replymsg(msg.id, "پاک کننده گیف(عکس متحرک) فعال گردید", ok_cb, false) 
end

elseif matches[2] == "fwd" then
if data[tostring(target)]["settings"]["lock_fwd"] == "yes" then
	return replymsg(msg.id, "پاک کننده پیام های فوروارد فعال است", ok_cb, false)
	else
    data[tostring(target)]["settings"]["lock_fwd"] = "yes"
    save_data(data)
	--savelog(msg.to.id, "پاک کننده پیام های فوروارد فعال گردید توسط "..msg.from.print_name:gsub("_",""))
    return replymsg(msg.id, "پاک کننده پیام های فوروارد فعال گردید", ok_cb, false)
end

elseif matches[2] == "bot" then
if data[tostring(target)]["settings"]["lock_bot"] == "yes" then
    channel_get_bots(receiver, clean_bots3, {msg = msg})
	return replymsg(msg.id, "ضد ربات فعال است", ok_cb, false)
	else
    data[tostring(target)]["settings"]["lock_bot"] = "yes"
    save_data(data)
	--savelog(msg.to.id, "ضد ربات فعال گردید توسط "..msg.from.print_name:gsub("_",""))
	channel_get_bots(receiver, clean_bots3, {msg = msg})
    return replymsg(msg.id, "ضد ربات فعال گردید", ok_cb, false)
end

elseif matches[2] == "join" then
if data[tostring(target)]["settings"]["lock_join"] == "yes" then
	return replymsg(msg.id, "پاک کننده افراد جوین دهنده با لینک فعال است", ok_cb, false)
	else
    data[tostring(target)]["settings"]["lock_join"] = "yes"
    save_data(data)
	--savelog(msg.to.id, "پاک کننده افراد جوین دهنده با لینک فعال گردید توسط "..msg.from.print_name:gsub("_",""))
    return replymsg(msg.id, "پاک کننده افراد جوین دهنده با لینک فعال گردید", ok_cb, false)
end

elseif matches[2] == "reply" then
if data[tostring(target)]["settings"]["lock_reply"] == "yes" then
	return replymsg(msg.id, "پاک کننده پیام های ریپلی(پاسخ) فعال است", ok_cb, false)
	else
    data[tostring(target)]["settings"]["lock_reply"] = "yes"
    save_data(data)
	--savelog(msg.to.id, "پاک کننده پیام های ریپلی(پاسخ) فعال گردید توسط "..msg.from.print_name:gsub("_",""))
    return replymsg(msg.id, "پاک کننده پیام های ریپلی(پاسخ) فعال گردید", ok_cb, false)
end

elseif matches[2] == "tag" then
if data[tostring(target)]["settings"]["lock_tag"] == "yes" then
	return replymsg(msg.id, "پاک کننده پیام های دارای تگ فعال است", ok_cb, false)
	else
    data[tostring(target)]["settings"]["lock_tag"] = "yes"
    save_data(data)
	--savelog(msg.to.id, "پاک کننده  پیام های دارای تگ فعال گردید توسط "..msg.from.print_name:gsub("_",""))
    return replymsg(msg.id, "پاک کننده پیام های دارای تگ فعال گردید", ok_cb, false)
end

elseif matches[2] == "media" then
if data[tostring(target)]["settings"]["lock_media"] == "yes" then
	return replymsg(msg.id, "پاک کننده مدیا(هرچیزی غیر از متن) فعال است", ok_cb, false)
	else
    data[tostring(target)]["settings"]["lock_media"] = "yes"
    save_data(data)
	--savelog(msg.to.id, "پاک کننده مدیا(هرچیزی غیر از متن) فعال توسط "..msg.from.print_name:gsub("_",""))
    return replymsg(msg.id, "پاک کننده مدیا(هرچیزی غیر از متن) فعال گردید", ok_cb, false)
end
...
if data[tostring(target)]["settings"]["lock_capmedia"] == "yes" then
    ...
    data[tostring(target)]["settings"]["lock_capmedia"] = "yes"
    save_data(data)
	savelog(msg.to.id, "پاک کننده پیام های دارای عنوان(کپشن) فعال توسط "..msg.from.print_name:gsub("_",""))
    return replymsg(msg.id, "پاک کننده پیام های دارای عنوان(کپشن) فعال گردید", ok_cb, false)
end

elseif matches[2] == "porn" then
if data[tostring(target)]["settings"]["lock_porn"] == "yes" then
	return replymsg(msg.id, "پاک کننده تصاویر +18 فعال است", ok_cb, false)
	else
    data[tostring(target)]["settings"]["lock_porn"] = "yes"
    save_data(data)
	savelog(msg.to.id, "پاک کننده پیام های دارای عنوان(کپشن) فعال توسط "..msg.from.print_name:gsub("_",""))
    return replymsg(msg.id, "پاک کننده تصاویر +18 فعال گردید", ok_cb, false)
end
...
if data[tostring(target)]["settings"]["lock_unsup"] == "yes" then
	return replymsg(msg.id, "پاک کننده پیام های ناشناخته فعال است", ok_cb, false)
	else
    ...
    return replymsg(msg.id, "پاک کننده پیام های ناشناخته فعال گردید", ok_cb, false)
   end
   end
end
--------------------------------------------------#echo 
...
if type(msg.reply_id) ~= "nil" then
		 local function vardump_reply2(extra , success, result)
          return replymsg(extra.msg.id, serpent.block(result, {comment = false}), ok_cb, false)
        end
		  get_message(msg.reply_id, vardump_reply2, {msg = msg})
else
   return replymsg(msg.id, serpent.block(msg, {comment = false}), ok_cb, false)
end
end
if matches[1] == "vardump2" then
if type(msg.reply_id) ~= "nil" then
		 local function vardump_reply2(extra , success, result)
          return replymsg(extra.msg.id, vardump2(result), ok_cb, false)
        end
		  get_message(msg.reply_id, vardump_reply2, {msg = msg})
else
   return replymsg(msg.id, serpent.block(msg, {comment = false}), ok_cb, false)
...
--------------------------------------------------#welcome settings
 ...
 if matches[1] == "delwlc" and is_momod(msg) then
        ...
        return replymsg(msg.id, "متن پیام خوش امدگویی حذف شد", ok_cb, false)
end
if matches[1] == "wlc" and not matches[2] and is_momod(msg) then
        if data[tostring(chat_id)]["settings"]["wlc"] == "off" then
		   return replymsg(msg.id, "پیام خوش امدگویی غیرفعال است\nبا <b>WLC ON </b> فعال کنید", ok_cb, false)
		end
		--savelog(msg.to.id, "متن پیام خوش امدگویی درخواست شد توسط توسط "..msg.from.print_name:gsub("_",""))
		 group_wlc = string.gsub(data[tostring(chat_id)]["group_wlc"], "$name", "'اسم فرد وارد شده'")
         group_wlc = string.gsub(group_wlc, "$gname", "'اسم گروه'")
         group_wlc = string.gsub(group_wlc, "$uname", "'نام کاربری فرد وارد شده'")
		 group_wlc = string.gsub(group_wlc, "$2name", "'اسم واسطه'")
		 group_wlc = string.gsub(group_wlc, "$u2name", "'یوزرنیسم واسطه'")
         group_wlc = string.gsub(group_wlc, "$rules", "\n'قوانین:'\n\n")
		 group_wlc = string.gsub(group_wlc, "$about", "\n'توضیحات:'\n\n")
        return replymsg(msg.id, group_wlc, ok_cb, false)
end
--------------------------------------------------#warn
if matches[1] == "warn" and is_momod(msg) then
    if type(msg.reply_id)~="nil" then
      local function warn_reply(extra , success, result)
		  ...
		  local chat_id = result.to.peer_id
		  if is_momod2(user_id, chat_id) then
		  return replymsg(msg.id, names.." ایشان دارای مقام است", ok_cb, false)
		  end
...
if matches[1] == "unwarnall" and is_momod(msg) then
    if type(msg.reply_id)~="nil" then
      local function unwarnall_reply(extra , success, result)
		  ...
		  local warns2 = redis:get(warns)
		  if not warns2 then
		   return replymsg(extra.msg.reply_id, names.." تاکنون اخطاری دریافت نکرده است", ok_cb, false)
		else
		   if tonumber(warns2) == 0 then
		    return replymsg(extra.msg.reply_id, names.." تاکنون اخطاری دریافت نکرده است", ok_cb, false)
		   end
...
elseif matches[1] == "unwarnall" and matches[2] and string.match(matches[2], "^%d+$") then
		  ...
		  local warns2 = redis:get(warns)
        if not warns2 then
		   return replymsg(msg.id, user_id.." تاکنون اخطاری دریافت نکرده است", ok_cb, false)
		else
		   if tonumber(warns2) == 0 then
		    return replymsg(msg.id, user_id.." تاکنون اخطاری دریافت نکرده است", ok_cb, false)
		   end
      ...
      elseif matches[1] == "unwarnall" and matches[2] and not string.match(matches[2], "^%d+$") then
		 ...
		 local user_id = result.peer_id
		  ...
		  local warns2 = redis:get(warns)
		  if not warns2 then
		   return replymsg(msg.id, names.." تاکنون اخطاری دریافت نکرده است", ok_cb, false)
		else
		   if tonumber(warns2) == 0 then
		    return replymsg(msg.id, names.." تاکنون اخطاری دریافت نکرده است", ok_cb, false)
		   end
...
return sendmsg(receiver, "محدودیت اخطار ها به کاربر<b> "..mwarn.." </b>میباشد")
end
...
if matches[1] == "settings" and matches[2] == "unlock" then
   ...
   return replymsg(msg.id, "تنظیمات سخت گیرانه غیرفعال شد", ok_cb, false)
  end
end
--------------------------------------------------#HTML
if matches[1] == "bold" then
local mtn = msg.text:gsub("bold", "")
  return replymsg(msg.id, "متن شما :\n\n <b>"..mtn.."</b>", ok_cb, false)
   ...
   if not is_sudo(msg) then
    ...
    if type(msg.reply_id)~="nil" then
      local function hammer_reply(extra , success, result)
		  ...
		  return replymsg(extra.msg.id, names.." ایشان داری مقام ادمین است", ok_cb, false)
		  end
          if is_divest(user_id) then
		  return replymsg(extra.msg.reply_id, names.." در لیست خلع وجود دارد", ok_cb, false)
		  end
		  block_user("user#id"..user_id, ok_cb, false)
          divest(user_id)
		  kick_user(user_id, group_id)
    ...
    elseif matches[1] == "hammer" and matches[2] and string.match(matches[2], "^%d+$") then
        ...
        if is_divest(matches[2]) then
		  return replymsg(msg.id, matches[2].." در لیست خلع وجود دارد", ok_cb, false)
		end
		 block_user("user#id"..matches[2], ok_cb, false)
        divest(matches[2])
		kick_user(matches[2], msg.to.id)
		sendmsg(receiver, "[ "..matches[2].." ] از گروه های یوبی محروم شد")
      elseif matches[1] == "hammer" and matches[2] and not string.match(matches[2], "^%d+$") then
		 ...
		 if success == 0 then
		return sendmsg(receiver, "❌ یوزرنیم مورد نظر یافت نشد یا اشتباه است ❌")
	  end
		 local names = "@"..result.username
		 local user_id = result.peer_id
		  if is_admin2(user_id, group_id) then
		  ...
		  if is_divest(user_id) then
		  return replymsg(extra.msg.id, names.." در لیست خلع وجود دارد", ok_cb, false)
		  end
		   block_user("user#id"..user_id, ok_cb, false)
		 divest(user_id)
		 kick_user(user_id, extra.chat_id)
   ...
   if not is_sudo(msg) then
    ...
    if type(msg.reply_id)~="nil" then
      local function unhammer_reply(extra , success, result)
		  ...
		  return replymsg(extra.msg.reply_id, names.." در لیست خلع وجود ندارد", ok_cb, false)
		  end
		   unblock_user("user#id"..user_id, ok_cb, false)
          undivest(user_id)
    ...
    elseif matches[1] == "unhammer" and matches[2] and string.match(matches[2], "^%d+$") then
        if not is_divest(matches[2]) then
		  return replymsg(msg.id, matches[2].." در لیست خلع وجود ندارد", ok_cb, false)
		  end
		   unblock_user("user#id"..matches[2], ok_cb, false)
        undivest(matches[2])
		sendmsg(receiver, "[ "..matches[2].." ] از خلع خارج شد")
      elseif matches[1] == "unhammer" and matches[2] and not string.match(matches[2], "^%d+$") then
		...
		return sendmsg(receiver, "❌ یوزرنیم مورد نظر یافت نشد یا اشتباه است ❌")
		 ...
		 local user_id = result.peer_id
		  if not is_divest(user_id) then
		  return replymsg(extra.msg.id, names.." در لیست خلع وجود ندارد", ok_cb, false)
		  end
		   block_user("user#id"..user_id, ok_cb, false)
		 undivest(user_id)
...
if matches[1] == "whois" and matches[2] and string.match(matches[2], "^%d+$") then
	...
	if success == 0 then
	return replymsg(extra.msg.id, "شناسه نامعتبر است", ok_cb, false)
   end
		if result.username then
		 ...
		 return replymsg(extra.msg.id, names, ok_cb, false)
   end
		 local user_id = "user#id"..matches[2]
...
if matches[1] == "kickmember" and is_momod(msg) and redis:get("kmember"..bot_divest..":"..msg.to.id) == nil then
       ...
       return sendmsg(receiver, "حالا پیام شخص مورد نظر را فوروارد کنید")
elseif matches[1] == "kickmember" and is_momod(msg) and redis:get("kmember"..bot_divest..":"..msg.to.id) == "waite" then
       return sendmsg(receiver,"شما قبلا در خواست کرده ایید لطفا پیام کاربر مورد نظر را فور وراد کنید دهید" )
end
--------------------------------------------------#setuwername
if matches[1] == "setusername" and is_admin1(msg) then
			local function ok_username_cb (extra, success, result)
				if success == 1 then
					sendmsg(receiver, "انجام شد")
				elseif success == 0 then
					sendmsg(receiver, "خطا")
				end
			end
			local username = string.gsub(matches[2], "@", "")
...
و اگر ریپورت هستید با @UBsupportbot در ارتباط باشید

رباتهای داعمی :
بات های معمولی به قیمت 10000 تومان 
بات های ویژه به قیمت 15000 تومان 
بات های VIP به قیمت 20000 تومان
...
<code>ربات ماهانه از رباتهای VIP میباشد </code>
...
redis:sadd(users, msg.from.id)
if msg.reply_id then
return replymsg(msg.reply_id, text, ok_cb, false)
else
return replymsg(msg.id, text, ok_cb, false)
end
end
...
if matches[1] == "feedback" and not msg.fwd_from then
      ...
      for v,user in pairs(config.sudo_users) do
          --sendmsg("user#id"..user, matches[2])
          fwd_msg("user#id"..user, msg.id, ok_cb, false)
      end
	  --redis:setex("feedback:"..msg.from.id, (3600 * 12), true)
	  return sendmsg(receiver, "پیام شما برای سازنده ارسال شد\nبا تشکر از نظرات شما")
end
if matches[1] == "trafic" and is_admin1(msg) then
return "ترافیک ربات:\n"..redis:get("msgs11"..bot_divest).." در یک ثانیه\n"..redis:get("msgs22"..bot_divest).." در 5 ثانیه\n"..redis:get("msgs33"..bot_divest)
     .." در 10 ثانیه"--.."\nبیشترین ترافیک ربات در 1 ثانیه:\n"..redis:get("msgsc"..bot_divest)
end
--------------------------------------------------#info
...
elseif tonumber(count) > 0 then
      ...
      local count = jdat.result.total_count
      local RES, code = https.request(path..fileid)
      local jdat2 = json:decode(RES)
...
if matches[1] == "info" and msg.reply_id and not matches[2] then
...
elseif tonumber(count) > 0 then
   ...
   profs = "\nProfs: "..count
     ...
     send_photo2(chat_id, photo, cap, ok_cb, false)
end
...
elseif tonumber(count) > 0 then
  ...
  if count == 0 then
   ...
   profs = "\nProfs: "..count
     ...
     local username = matches[2]
	 local username = string.gsub(matches[2], "@", "")
     resolve_username(username, info_username, {username = username, type = type})

end
if matches[1] == "msgid" and msg.reply_id and is_admin1(msg) then
function msgid_reply(extra , success, msg)
         return replymsg(extra.msg.id, msg.id)
end
     msgr = get_message(msg.reply_id, msgid_reply, {msg = msg})

end
if matches[1] == "fwd" and matches[2] and is_admin1(msg) then
   return fwd_msg(receiver, matches[2], ok_cb, false)
end
if matches[1] == "setadds" and matches[2] and is_sudo(msg) then
   redis:sadd("adds"..bot_divest, matches[2])
   return replymsg(msg.id, "New Adds added to list adds")
end
if matches[1] == "remadds" and matches[2] and is_sudo(msg) then
   redis:srem("adds"..bot_divest, matches[2])
   return replymsg(msg.id, "New Adds removed from list adds")
end
if matches[1] == "adds" and is_sudo(msg) then
   adds = ""
   i = 1
   list = redis:smembers("adds"..bot_divest)
   for k,v in pairs(list) do
       adds = adds..i.." - "..v.."\n"
	   i = i + 1
   end
   return replymsg(msg.id, adds)
end
if matches[1] == "cleanadds" and is_sudo(msg) then
   redis:del("adds"..bot_divest)
   return replymsg(msg.id, "Addslist is clear")
end
if matches[1] == "setlang fa" and is_owner1(msg) then
   redis:set("lang"..msg.to.id, "fa")
   return replymsg(msg.id, "زبان ربات در گروه فارسی گردید")
end
if matches[1] == "setlang en" and is_owner1(msg) then
   redis:set("lang"..msg.to.id, "en")
   return replymsg(msg.id, "Robot language has been set to English")
end
--------------------------------------------------#
...
function cron()
    ...
    kicktable = {}
	local kickmes = redis:smembers("kickmes"..bot_divest)
	for k,v in pairs(kickmes) do
	    if redis:get("kickme"..bot_divest..v) then
		   redis:del("kickme"..bot_divest..v, true)
		   redis:srem("groupmuteall"..bot_divest, v)
		end
	end
	for k,v in pairs(redis:smembers("groupmuteall"..bot_divest)) do
	    if not redis:get("muteall:"..v) then
		   send_msg(v, "<code>مدت زمان پاک کننده همه پیام ها به اتمام رسید </code>\n\n<i>هم اکنون گروه برای ارسال مطالب آزاد است </i>", ok_cb, false)
		   redis:srem("groupmuteall"..bot_divest, v)
		end
	end
	local list = redis:smembers("testgroups"..bot_divest)
	for k,v in pairs(list) do
	 if data[tostring("tests")] then
	    group = string.gsub(v, "channel#id", "")
	    if data[tostring("tests")][tostring(group)] then
		   if not redis:get("limit"..bot_divest..v) then
			  sendmsg(v, "مدت آزمایش شما از ربات تمام شد\nلذا حتما به ادمینی که درخواست تست کرده اید مراجعه کنید")
			  sendmsg("user#id"..redis:hget("testgroup"..bot_divest..group, "testgroup"), "مدت آزمایش ربات در "..group.." به اتمام رسید ")
              redis:del("chat:"..group..":badword", "1")
	          redis:del("Groupcm"..bot_divest..":"..group, true)
	          redis:del("mwarn"..bot_divest..":"..group, true)
			  data[tostring(group)] = nil
			  save_data(data)
			  data[tostring("tests")][tostring(group)] = nil
			  save_data(data)
			  redis:hdel("testgroup"..bot_divest..group, "testgroup")
			  leave_channel(v, ok_cb, false)
			  return redis:srem("testgroups"..bot_divest, v)
		   end
	   end
     end
   end
   ...
   now = os.time()
    ...
    local values = values[1][2]
	  for v,user in pairs(config.sudo_users) do
          sendmsg("user#id"..user, "شارژ این گروه تمام شد "..string.gsub(receiver, "channel#id", "").." شارژ شده توسط "..redis:hget(receiver, "expire"))
      end
 	  	sendmsg(receiver, "به نظر میرسد شارژ گروه شما به اتمام رسیده است \nخواهش مند است نسبت به تمدید اقدام کنید\n"
		.."شما باید به ادمینی که ازش رباتو خریدید مراجعه کنید یا باید به پی وی این ربات امده و لینک گروه پشتیبانی را دریافت کنید\n"
		.."<code>ربات تا زمان تمدید غیرفعال میشود</code>\n"..string.gsub(redis:hget(receiver, "expire"), "@", "(at)"))
		sendmsg(values, "شارژ این گروه تمام شد "..string.gsub(receiver, "channel#id", ""))
...
local function pre_process(msg)
  if msg.to.type == "chat" and not data[tostring(msg.to.id)] then
     redis:sadd("chat:"..msg.to.id..":users", msg.from.id)
  end

  if msg.to.type == "channel" and not data[tostring(msg.to.id)] then
     redis:sadd("channel:"..msg.to.id..":users", msg.from.id)
  end

if data[tostring(msg.to.id)] and msg.to.type == "channel" and msg.text and msg.text:lower() == "bot on" and is_owner(msg) then
   return enable_channel(receiver)
elseif data[tostring(msg.to.id)] and msg.to.type == "channel" and msg.text and msg.text:lower() == "bot off" and is_owner(msg) then
   return disable_channel(receiver)
end


  if is_channel_disabled(receiver) and data[tostring(msg.to.id)] then
      if msg.text and msg.to.type == "channel" and is_momod(msg) then
         if is_pattern(msg) then
            return sendmsg(receiver, "لیدر عزیز ربات غیرفعال است\nمیتوانید با <b>BOT ON</b> فعال کنید")
	     end
	  end
  	return false
  end
...
if msgs >= NUM_MSG_MAX then
   ...
   if not kicktable[msg.from.id] == true then
	  kick_user(msg.from.id, msg.to.id)
	  if msg.from.username ~= nil then username = "@"..msg.from.username else username = string.sub(msg.from.print_name:gsub("_"," "), 1, 100) end
	  sendmsg(receiver, "کاربر "..username.." به دلیل ارسال پیام های مکرر بیش از "..NUM_MSG_MAX.." بار ریمو شد")
...
redis:setex(hash, TIME_CHECK, msgs + 1) --flood_settings

----------------------------------------------------------------#check messages
if msg.to.type == "channel" and data[tostring(msg.to.id)] and not is_momod(msg) then
if ( is_muted_user(msg.to.id, msg.from.id) or is_muteall_user(msg.from.id) or redis:get("muteall:"..receiver) ) then
   delete_msg(msg.id, ok_cb, false)
   return false
end
settings = data[tostring(msg.to.id)]["settings"]
 if (settings.lock_bot == "yes" or settings.lock_bots) == "yes" then
    if msg.from.username ~= nil and string.sub(msg.from.username:lower(), -3) == "bot" then
	   delmsg(msg.id)
	   kick_user(msg.from.id, msg.to.id)
	   return false
    end
...
local warn2 = redis:get(gwarns)
local gwarn = tonumber(redis:get(gwarns))
if settings.lock_settings == "yes" then
   if warn2 then
      if gwarn >= tonumber(mwarn) then
		 ...
		 return sendmsg(receiver, "کاربر "..string.gsub(msg.from.print_name, "_", " ").." به دلیل عدم رعایت قوانین ریمو شد")
		 end
      end
   end

if settings.lock_fwd == "yes" and msg.fwd_from then
	if settings.lock_settings == "yes" then
	    redis:incr(gwarns)
    end
	    delmsg(msg.id)
		return false
elseif settings.lock_fwd == "kick" and msg.fwd_from then
	    delmsg(msg.id)
		return kick_user(msg.from.id, msg.to.id)
end

if settings.lock_reply == "yes" and msg.reply_id then
    if settings.lock_settings == "yes" then
	    redis:incr(gwarns)
    end
		return delmsg(msg.id)
elseif settings.lock_reply == "kick" and msg.reply_id then
	    delmsg(msg.id)
		return kick_user(msg.from.id, msg.to.id)
end
...
if settings.lock_text == "yes" then
	if settings.lock_settings == "yes" then
		...
		return delmsg(msg.id)
elseif settings.lock_text == "kick" then
	    delmsg(msg.id)
		return kick_user(msg.from.id, msg.to.id)
end
local hash = "chat:"..msg.to.id..":badword"
    local names = redis:hkeys(hash)
    for i=1, #names do --3
	  if string.match(text:lower(), names[i]) then --4
           return delmsg(msg.id)
    end
  end
local is_link_msg = ((text:lower()):match("(telegram.me/%S+)") or (text:lower()):match("(tlgrm.me/%S+)"))
	if settings.lock_link == "yes" and is_link_msg then
	if settings.lock_settings == "yes" then
	    redis:incr(gwarns)
    end
		return delmsg(msg.id)
elseif settings.lock_link == "kick" and is_link_msg then
	    delmsg(msg.id)
		return kick_user(msg.from.id, msg.to.id)
end
...
elseif settings.lock_spam == "yes" and text:match("[a-zA-Z]") and string.len(text) > 4000 then
    if settings.lock_settings == "yes" then
	    ...
	    return delmsg(msg.id)
elseif settings.lock_spam == "kick" and is_fars_msg and string.len(text) > 6000 then
	    delmsg(msg.id)
		return kick_user(msg.from.id, msg.to.id)
elseif settings.lock_spam == "kick" and text:match("[a-zA-Z]") and string.len(text) > 4000 then
	    delmsg(msg.id)
	    return kick_user(msg.from.id, msg.to.id)
end

if settings.lock_spam == "yes" and data[tostring(msg.to.id)]["settings"]["lock_numspam"] and is_fars_msg and string.len(text) > NUM_LEN_MAX then
	if settings.lock_settings == "yes" then
	    redis:incr(gwarns)
    end
	    return delmsg(msg.id)
elseif settings.lock_spam == "yes" and data[tostring(msg.to.id)]["settings"]["lock_numspam"] and msg.text:match("[a-zA-Z]") and string.len(text) > NUM_LEN_MAX then
    if settings.lock_settings == "yes" then
	    redis:incr(gwarns)
    end
	    return delmsg(msg.id)
elseif settings.lock_spam == "kick" and data[tostring(msg.to.id)]["settings"]["lock_numspam"] and string.len(msg.text) > NUM_LEN_MAX then
	    delmsg(msg.id)
		return kick_user(msg.from.id, msg.to.id)
elseif settings.lock_spam == "kick" and data[tostring(msg.to.id)]["settings"]["lock_numspam"] and string.len(msg.text) > NUM_LEN_MAX then
	    delmsg(msg.id)
...
if settings.lock_arabic == "yes" and is_fars_msg then
	if settings.lock_settings == "yes" then
	    redis:incr(gwarns)
    end
		return delmsg(msg.id)
elseif settings.lock_arabic == "kick" and is_fars_msg then
	    delmsg(msg.id)
		return kick_user(msg.from.id, msg.to.id)
end
local is_tag_msg = (text:match("#[%a%d]") or text:match("@[%a%d]"))
if settings.lock_tag == "yes" and is_tag_msg then
	if settings.lock_settings == "yes" then
	    redis:incr(gwarns)
    end
		return delmsg(msg.id)
elseif settings.lock_tag == "kick" and is_tag_msg then
	    delmsg(msg.id)
		return kick_user(msg.from.id, msg.to.id)
end
--"[a|b|c|d|e|f|g|h|i|j|k|l|m|n|o|p|q|r|s|t|u|v|w|x|y|z]""[a-zA-Z]"
local is_en_msg = text:match("[a-zA-Z]")
if settings.lock_en == "yes" and is_en_msg then
	if settings.lock_settings == "yes" then
	    redis:incr(gwarns)
    end
		return delmsg(msg.id)
elseif settings.lock_en == "kick" and is_en_msg then
	    delmsg(msg.id)
		return kick_user(msg.from.id, msg.to.id)
end
end
...
if settings.lock_media == "yes" then
    if settings.lock_settings == "yes" then
      ...
      return delmsg(msg.id)
elseif settings.lock_media == "kick" then
      delmsg(msg.id)
      return kick_user(msg.from.id, msg.to.id)
end
if msg.media.caption then
caption = msg.media.caption:lower()
if settings.lock_capmedia == "yes" then
    if settings.lock_settings == "yes" then
	    redis:incr(gwarns)
    end
      return delmsg(msg.id)
elseif settings.lock_capmedia == "kick" then
      delmsg(msg.id)
      return kick_user(msg.from.id, msg.to.id)
end
   ...
   local hash = "chat:"..msg.to.id..":badword"
             local names = redis:hkeys(hash)
             for i=1, #names do --4
                if string.match(caption, names[i]) then --6
                   return delmsg(msg.id)
                end
             end
local is_link_cap = ((caption:lower()):match("(telegram.me/%S+)") or (caption:lower()):match("(tlgrm.me/%S+)"))
if settings.lock_link == "yes" and is_link_cap then
    if settings.lock_settings == "yes" then
	    redis:incr(gwarns)
    end
      return delmsg(msg.id)
elseif settings.lock_link and lock_link == "kick" then
      delmsg(msg.id)
      return kick_user(msg.from.id, msg.to.id)
end
local is_tag_cap = (caption:match("@[%a%d]") or caption:match("#[%a%d]"))
if settings.lock_tag == "yes" and is_tag_cap then
    if settings.lock_settings == "yes" then
	    redis:incr(gwarns)
    end
      return delmsg(msg.id)
elseif settings.lock_tag == "kick" and is_tag_cap then
      delmsg(msg.id)
      return kick_user(msg.from.id, msg.to.id)
end
local is_fars_msg = caption:match("[\216-\219][\128-\191]")
if settings.lock_arabic == "yes" and is_fars_cap then
    if settings.lock_settings == "yes" then
	    redis:incr(gwarns)
    end
      return delmsg(msg.id)
elseif settings.lock_arabic == "kick" and is_fars_cap then
      delmsg(msg.id)
      return kick_user(msg.from.id, msg.to.id)
end
local is_en_msg = caption:match("[a-z]")
if settings.lock_en == "yes" and is_en_cap then
    if settings.lock_settings == "yes" then
	    redis:incr(gwarns)
    end
      return delmsg(msg.id)
elseif settings.lock_en == "kick" and is_en_cap then
      delmsg(msg.id)
      return kick_user(msg.from.id, msg.to.id)
end
...
if msg.media.name then
caption = msg.media.name:lower()

local is_sticker = caption:match("sticker.webp") and msg.media.type == "document"
if settings.lock_sticker == "yes" and is_sticker then
    if settings.lock_settings == "yes" then
	    redis:incr(gwarns)
    end
      return delmsg(msg.id)
elseif settings.lock_sticker == "kick" and is_sticker then
      delmsg(msg.id)
      return kick_user(msg.from.id, msg.to.id)
end
local is_gif = caption:match(".mp4") and msg.media.type == "document"
if settings.lock_gif == "yes" and is_gif then
    if settings.lock_settings == "yes" then
	    redis:incr(gwarns)
    end
      return delmsg(msg.id)
elseif settings.lock_gif == "kick" and is_gif then
      delmsg(msg.id)
      return kick_user(msg.from.id, msg.to.id)
end
end
...
if lock_photo == "yes" and is_photo then
    if settings.lock_photo == "yes" then
		...
		return delmsg(msg.id)
elseif settings.lock_photo == "kick" and is_photo then
	    delmsg(msg.id)
		return kick_user(msg.from.id, msg.to.id)
end
local is_file = msg.media.type == "document" 
if settings.lock_document == "yes" and is_file then
   if msg.media.name and msg.media.name:lower() == ("sticker.webp" or ".mp4") then
      return msg
   end
    if settings.lock_settings == "yes" then
	    redis:incr(gwarns)
		...
		return delmsg(msg.id)
elseif settings.lock_document == "kick" and is_file then
       if msg.media.name and msg.media.name:lower() == ("sticker.webp" or ".mp4") then
      return msg
   end
	    delmsg(msg.id)
...
if settings.lock_audio == "yes" and is_audio then
    if settings.lock_settings == "yes" then
	    redis:incr(gwarns)
    end
		return delmsg(msg.id)
elseif settings.lock_audio == "kick" and is_audio then
	    delmsg(msg.id)
		return kick_user(msg.from.id, msg.to.id)
end
local is_video = msg.media.type == "video"
if settings.lock_video == "yes" and is_video then
    if settings.lock_settings == "yes" then
	    redis:incr(gwarns)
    end
      return delmsg(msg.id)
elseif settings.lock_video == "kick" and is_video then
      delmsg(msg.id)
      return kick_user(msg.from.id, msg.to.id)
end
local is_share = msg.media.type == "contact"
if settings.lock_contact == "yes" and is_share then
    if settings.lock_settings == "yes" then
	    redis:incr(gwarns)
    end
		return delmsg(msg.id)
elseif settings.lock_contact == "kick" and is_share then
	    delmsg(msg.id)
		return kick_user(msg.from.id, msg.to.id)
end
...
if settings.lock_unsup == "yes" then
	if settings.lock_settings == "yes" then
		...
		return delmsg(msg.id)
elseif settings.lock_unsup == "kick" then
	    delmsg(msg.id)
		return kick_user(msg.from.id, msg.to.id)
end
end
if settings.lock_porn == "yes" and is_photo then
   load_photo(msg.id, nuditycheck, msg)
end

if settings.lock_porn == "yes" and msg.media.name and (msg.media.name:lower()):match("sticker.webp") and msg.media.type == "document" then
   load_document(msg.id, nuditycheck, msg)
end
...
------------------------------------------------#implementation settings
  ...
  if msg.text then
        if msg.text:match("[Yy][Ee][Ss]") then
		   if redis:get("kickme"..bot_divest..msg.from.id) then
		      if not msg.from.username then
			     usernames = msg.from.print_name:gsub("_", " ")
			  else
			     usernames = "@"..msg.from.username
			  end
		      delmsg(msg.id)
			  kick_user(msg.from.id, msg.to.id)
			  redis:del("kickme"..bot_divest..msg.to.id, true)
			  return sendmsg(receiver, "کاربر "..usernames.." به درخواست خود از گروه اخراج گردید")
		   end
		end
	    ...
	    if data[tostring(msg.to.id)] and msg.text:match("^[Ss][Ee][Tt][Ll][Ii][Nn][Kk]$") or msg.text:match("^[!/#][Ss][Ee][Tt][Ll][Ii][Nn][Kk]$") and is_momod(msg) then
			...
			return replymsg(msg.id, "لطفا متن دارای لینک خود را ارسال کنید", ok_cb, false)
		end
 		if data[tostring(msg.to.id)] and msg.text:match("(https://telegram.me/joinchat/%S+)") and data[tostring(msg.to.id)]["settings"]["set_link"] == "waiting" and is_momod(msg) then
				...
				return replymsg(msg.id, "بسیار خب لینک ثبت شد", ok_cb, false)
		end
		if data[tostring(msg.to.id)] and msg.text:match("^[Ss][Ee][Tt][Ll][Ii][Nn][Kk] (.*)") or msg.text:match("^[!/#][Ss][Ee][Tt][Ll][Ii][Nn][Kk] (.*)") and is_momod(msg) then
        ...
        if msg.media then
			...
			if msg.media.caption and is_momod(msg) then
                ...
                local is_link_cap = is_link_cap:match("^setphoto$") or is_link_cap:match("^[!/#]setphoto$")
				      if is_link_cap then
                         ...
                         local setphoto2 = redis:get(setphoto)
                         if setphoto2 then
                            return replymsg(msg.id, " مجاز به عوض کردن عکس در 1 ساعت 2 بار نیستید", ok_cb, false)
                         end
                         redis:setex(setphoto, 3600, true)
	  ...
	  if redis:get("kmember"..bot_divest..":"..msg.to.id) then
	     if redis:get("kmember"..bot_divest..":"..msg.to.id) == "waite" then
		   if is_momod2(msg.fwd_from.peer_id, msg.to.id) then
		      ...
		      return sendmsg(receiver, "ایشان "..ranks2(msg.fwd_from.peer_id, msg.to.id).." است")
		   end
	          redis:del("kmember"..bot_divest..":"..msg.to.id, true)
              kick_user(msg.fwd_from.peer_id, msg.to.id)
			  if not msg.fwd_from.username then
			     uname = msg.fwd_from.print_name:gsub("_", " ")
			  else
			     uname = "@"..msg.fwd_from.username
			  end
              return sendmsg(receiver, "❌کاربر "..uname.." اخراج شد")
          end
	   end
	end

if redis:get("addgroup"..bot_divest) == "on" then
if msg.to.type == "channel" and not data[tostring(msg.to.id)] and not is_admin1(msg) and redis:get("nilgroups:"..msg.to.id) then
   if tonumber(redis:get("nilgroups:"..msg.to.id)) >= 30 then
      redis:del("nilgroups:"..msg.to.id)
      leave_channel("channel#id"..msg.to.id, ok_cb, false)
   end
end

if msg.to.type == "channel" and not (is_super_group(msg) or is_test_group(msg)) and not is_admin1(msg) then
   redis:incr("nilgroups:"..msg.to.id)
   return false
end
...
if is_banned(msg.from.id, msg.to.id) then
	   if is_momod2(msg.from.id, msg.to.id) then
          unban_user(msg.from.id, msg.to.id)
	   end
	   if is_pattern(msg) then
	      replymsg(msg.id, "ظاهرا شما بن شده ایید\nاز یک فرد <code>مقام دار </code>بخواهید که از بن خارجتان کند")
	   end
       kick_user(msg.from.id, msg.to.id)
	   return false
    end
	if is_gbanned(msg.from.id) then
	   if is_momod2(msg.from.id, msg.to.id) then
          unbanall_user(msg.from.id)
	   end
	   if is_pattern(msg) then
	      replymsg(msg.id, "ظاهرا شما بن ال شده ایید\nاز <code>سازنده </code>بخواهید که از بن ال خارجتان کند")
	   end
       kick_user(msg.from.id, msg.to.id)
	   return false
  end
	if is_divest(msg.from.id) and not is_momod(msg) then
	   kick_user(msg.from.id, msg.to.id)
	   return false
	end
if redis:get("Groupcm"..bot_divest..":"..msg.to.id) == "on" and not is_momod(msg) then
  return false
end

        if msg.from.username ~= nil then
	       if string.sub(msg.from.username:lower(), -3) == "bot" and not is_owner1(msg) then
		      return false
		   end
		end
...
return {
  patterns = {
	...
	"^[#!/](lock) (all) (%d+)$","^(lock) (all) (%d+)$",
    "^[#!/](lock) (all) (%d+) (.*)","^(lock) (all) (%d+) (.*)",
    "^[#!/](stats) (lock)","^(stats) (lock)",
    "^[#!/](stats) (lock all)","^(stats) (lock all)",
    "^[#!/](mute)$","^(mute)$",
    "^[#!/](mute) (.*)$","^(mute) (.*)$",
    "^[#!/](unmute)$","^(unmute)$",
    "^[#!/](unmute) (.*)$","^(unmute) (.*)$",
