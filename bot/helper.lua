...
function is_owner2(user_id, group_id)
  ...
  if data[tostring(group_id)] then
    if data[tostring(group_id)]['owners'] then
      if data[tostring(group_id)]['owners'][tostring(user_id)] then
        var = true
      end
    end
  end
...
function is_momod2(user_id, group_id)
  ...
  if data[tostring(group_id)] then
    if data[tostring(group_id)]['owners'] then
      if data[tostring(group_id)]['owners'][tostring(usert)] then
        var = true
      end
    end
  end
...
lists = {}
	...
	if tonumber(#lists) == 0 then
	return "لیست افراد محروم از گروه خالی است"
	end
	local text = "لیست افراد محروم از گروه: [ID: "..chat_id.." ]:\n\n"
...
function banall_list()
	local hash =  'gbanned'
	local list = redis:smembers(hash)
	local text = "لیست افراد محروم از یوبی :\n\n"
	lists = {}
	for i,v in ipairs(list) do
		local p = v
		table.insert(lists, p)
	...
	if tonumber(#lists) == 0 then
	   return "لیست افراد محروم از یوبی خالی است"
	end
	for k,v in pairs(list) do
    local user_info = redis:hgetall('user:'..v)
		if user_info and user_info.print_name then
...
function muteall_list()
	...
	local text = "لیست افراد محروم از چت در گروه های یوبی :\n\n"
	for k,v in pairs(list) do
    local user_info = redis:hgetall('user:'..v)
		if user_info and user_info.print_name then
			local print_name = string.gsub(user_info.print_name, "_", " ")
...
function kick_list()
	local hash =  'kicked_user'
	local list = redis:smembers(hash)
	local text = "لیست افراد ریمو شده توسط یوبی!\n\n"
	lists = {}
	for i,v in ipairs(list) do
		local p = v
		table.insert(lists, p)
	...
	if tonumber(#lists) == 0 then
	   return "لیست افراد ریمو شده توسط یوبی خالی است"
	end
	text = "تعداد  افراد ریمو شده توسط یوبی "..#lists.." نفر میباشد"
	return text
end
...
function divestlist()
	local hash =  'divest'
	local list = redis:smembers(hash)
	local text = "لیست افراد خلع از یوبی :\n\n"
	lists = {}
	for i,v in ipairs(list) do
		local p = v
		table.insert(lists, p)
	...
	if tonumber(#lists) == 0 then
	   return "لیست اافراد خلع از یوبی خالی است"
	end
	for k,v in pairs(list) do
    local user_info = redis:hgetall('user:'..v)
		if user_info and user_info.print_name then
...
function unmuteall_user(user_id)
  local hash = 'muteall_user'
  redis:srem(hash, user_id)
end

function is_muteall_user(user_id)
  local hash = 'muteall_user'
  local banned = redis:sismember(hash, user_id)
  return banned or false
end

function divest(user_id)
  if is_admin2(user_id) then
    return "ادمین است!!!"
  end
  local hash = 'divest'
...
function is_divest(user_id)
  local hash = 'divest'
  local divest = redis:sismember(hash, user_id)
  return divest or false
end

function undivest(user_id)
  local hash = 'divest'
  redis:srem(hash, user_id)
end
...
function muted_user_list(chat_id)
	...
	if tonumber(#lists) == 0 then
	   return "لیست افراد محروم از چت  خالی است"
	end
	local text = "افراد غیر مجاز در چت: [ID: "..chat_id.." ]:\n\n"
